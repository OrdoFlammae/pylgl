...
#define LOGREASON(LEVEL, LIT, REASON0, REASON1, FMT, ARGS...)                  \
  do {                                                                         \
    ...
    if (TAG == DECISION)                                                       \
      ...
      fprintf(lgl->out, " %s binary clause %d %d", lglred2str(RED), (LIT),     \
              TMP);                                                            \
    } else if (TAG == TRNCS) {                                                 \
      fprintf(lgl->out, " %s ternary clause %d %d %d", lglred2str(RED), (LIT), \
              TMP, (REASON1));                                                 \
    } else {                                                                   \
      assert(TAG == LRGCS);                                                    \
...
#define ABORTIF(COND, FMT, ARGS...)                                            \
  do {                                                                         \
    if (!(COND))                                                               \
      break;                                                                   \
    fprintf(stderr, "*** API usage error of '%s' in '%s'", __FILE__,           \
            __FUNCTION__);                                                     \
    if (lgl && lgl->tid >= 0)                                                  \
...
#define ASSERT(COND)                                                           \
  do {                                                                         \
    if ((COND))                                                                \
      break;                                                                   \
    fprintf(stderr, "liblgl.a: %s:%d: %s: Lingeling Assertion `%s' failed.",   \
...
#define COVER(COND)                                                            \
  do {                                                                         \
    if (!(COND))                                                               \
      break;                                                                   \
    fprintf(stderr, "liblgl.a: %s:%d: %s: Coverage target `%s' reached.",      \
            __FUNCTION__, __LINE__, __FILE__, #COND);                          \
    if (lgl && lgl->tid >= 0)                                                  \
      fprintf(stderr, " (tid %d)", lgl->tid);                                  \
    fputc('\n', stderr);                                                       \
...
typedef struct AVar {
  ...
  unsigned donotbasicprobe : 1, donotsimpleprobe : 1, donotreelook : 1,
      donotsweep : 1;
#ifndef NLGLYALSAT
...
#ifndef NDEBUG
double lglflt2dbl(Flt a) { return lglmnt(a) * pow(2.0, lglexp(a)); }
#endif
...
static void lgldelstr(LGL *lgl, char *str) { DEL(str, strlen(str) + 1); }
...
void lglsetconsumeunits(LGL *lgl, void (*fun)(void *, int **, int **),
                        void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
void lglsetconsumedunits(LGL *lgl, void (*fun)(void *, int), void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
void lglsetproducecls(LGL *lgl, void (*fun)(void *, int *, int), void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
void lglsetconsumecls(LGL *lgl, void (*fun)(void *, int **, int *),
                      void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
void lglsetconsumedcls(LGL *lgl, void (*fun)(void *, int), void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
void lglsetmsglock(LGL *lgl, void (*lock)(void *), void (*unlock)(void *),
                   void *state) {
  REQINITNOTFORKED();
  lglinitcbs(lgl);
...
static void lglfitstk(LGL *lgl, Stk *s) { lglshrstk(lgl, s, lglcntstk(s)); }
...
static int lglpopstk(Stk *s) {
  assert(!lglmtstk(s));
  return *--s->top;
}

static int lgltopstk(Stk *s) {
  assert(!lglmtstk(s));
  return s->top[-1];
}
...
static LGL *lglnewlgl(void *mem, lglalloc alloc, lglrealloc realloc,
                      lgldealloc dealloc) {
...
static void lglsetmaxminscore(LGL *lgl) {
  ...
  if (oldmaxscore != lgl->maxscore)
    lglprt(lgl, 1, "[set-maximum-score] maximum score limit %s",
           lglscr2str(lgl, lgl->maxscore));
  if (oldminscore != lgl->minscore)
    lglprt(lgl, 1, "[set-minimum-score] minimum score limit %s",
...
static void lglsetscincf(LGL *lgl, int scincinc) {
  ...
  lglprt(lgl, 2,
         ...
         lgl->stats->setscincf.count, lglscr2str(lgl, lgl->scincf), scincinc,
...
LGL *lglminit(void *mem, lglalloc alloc, lglrealloc realloc,
              lgldealloc dealloc) {
  ...
  lgl = 0;
  ABORTIF(!alloc + !realloc + !dealloc != 0 &&
              !alloc + !realloc + !dealloc != 3,
...
LGL *lglmclone(LGL *orig, void *mem, lglalloc alloc, lglrealloc realloc,
               lgldealloc dealloc) {
...
LGL *lglclone(LGL *lgl) {
  ...
  return lglmclone(lgl, lgl->mem->state, lgl->mem->alloc, lgl->mem->realloc,
                   lgl->mem->dealloc);
}
...
void lglpcs(LGL *lgl, int mixed) {
  ...
  for (o = FIRSTOPT(lgl); o <= LASTOPT(lgl); o++) {
    ...
    if (range < 7) {
      ...
      printf("{%d,%d,%d,%d,%d}", o->min, (int)(o->min + (1 * range + 3) / 4),
             (int)(o->min + (2 * range + 3) / 4),
             (int)(o->min + (3 * range + 3) / 4), o->max);
    } else if (o->dflt == o->min + 1) {
      printf("{%d,%d,%d,%d}", o->min, o->dflt,
             (int)(o->dflt + (o->max - (int64_t)o->dflt) / 2), o->max);
    } else if (o->dflt + 1 == o->max) {
      printf("{%d,%d,%d,%d}", o->min,
             (int)(o->min + (o->dflt - (int64_t)o->min) / 2), o->dflt, o->max);
    } else {
      assert(o->dflt - o->min >= 2);
      assert(o->max - o->dflt >= 2);
      printf("{%d,%d,%d,%d,%d}", o->min,
             (int)(o->min + (o->dflt - (int64_t)o->min) / 2), o->dflt,
             (int)(o->dflt + (o->max - (int64_t)o->min) / 2), o->max);
    }
    printf("[%d]", o->dflt);
    if (printi)
      printf("i");
...
void *lglnextopt(LGL *lgl, void *current, const char **nameptr, int *valptr,
...
int lglgetoptminmax(LGL *lgl, const char *opt, int *min_ptr, int *max_ptr) {
  Opt *o = lgligetopt(lgl, opt);
  if (!o)
    return 0;
  if (min_ptr)
...
static void lglrescorevars(LGL *lgl) {
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
    ...
    LOG(3, "rescored variable %d from %s to %s", idx, lglscr2str(lgl, oldscore),
        lglscr2str(lgl, qv->score));
  }
  lgldreschedule(lgl);
  ...
  lglprt(lgl, 3, "[rescored-vars-%d] after %lld conflicts and %lld OTFS",
         lgl->stats->rescored.vars,
  ...
  lglprt(lgl, 3, "[rescored-vars-%d] old maximum score %s",
         lgl->stats->rescored.vars, lglscr2str(lgl, oldmaxscore));
  lglprt(lgl, 3, "[rescored-vars-%d] new maximum score %s",
         lgl->stats->rescored.vars, lglscr2str(lgl, newmaxscore));
  oldscinc = lgl->scinc;
  lgl->scinc = lglshflt(oldscinc, DEFSCOREXP);
  assert(lgl->scinc > 0);
  lglprt(lgl, 3, "[rescored-vars-%d] old score increment %s",
         lgl->stats->rescored.vars, lglscr2str(lgl, oldscinc));
  lglprt(lgl, 3, "[rescored-vars-%d] new score increment %s",
...
static void lglmelter(LGL *lgl) {
  ...
  if (lgl->limits->elm.pen || lgl->limits->blk.pen || lgl->limits->cce.pen) {
    lglprt(lgl, 1, "[melter] reset penalties: %d elm, %d blk, %d cce",
...
static int lgldruplig(LGL *lgl) {
  if (!lgl->opts->druplig.val)
    return 0;
  if (!lgl->druplig) {
    lgl->druplig = druplig_minit(lgl, (druplig_malloc)lglnew,
                                 (druplig_realloc)lglrsz, (druplig_free)lgldel);
...
static void lglbacktrack(LGL *lgl, int level) {
  ...
  if (lgl->alevel > level) {
    LOG(2, "resetting assumption decision level to %d from %d", level,
        lgl->alevel);
    lgl->alevel = level;
    if (lgl->assumed) {
...
static int lglisimpcls(LGL *lgl) {
  ...
  *q = 0;
  ...
  if (satisfied)
    ...
    LOGCLS(2, lgl->clause.start, "%s simplified clause",
           simplified ? "changed" : "unchanged");
...
static void lglfreewch(LGL *lgl, unsigned oldoffset, int oldhcount) {
  ...
  LOG(5, "saving watch stack at %u of size %d on free list %d", oldoffset,
      oldhcount, ldoldhcount);
}
...
static ptrdiff_t lglenlwchs(LGL *lgl, HTS *hts) {
  ...
  LOG(5, "increasing watch stack at %u from %d to %d", oldoffset, oldhcount,
      newhcount);
  ...
  if (newoffset != UINT_MAX) {
    ...
    assert(lgl->wchs->free > 0);
    lgl->wchs->free--;
    LOG(5, "reusing free watch stack at %u of size %d", newoffset,
        (1 << ldnewhcount));
  } else {
    ...
    if (newwsize > oldwsize) {
      ...
      LOG(3, "resized global watcher stack from %ld to %ld", oldwsize,
          newwsize);
    ...
    LOG(5, "new watch stack of size %d at end of global watcher stack at %u",
        newhcount, newoffset);
  }
...
static ptrdiff_t lglwchlrg(LGL *lgl, int lit, int other, int red, int lidx) {
...
#ifndef NLGLOG
  {
    ...
    if (red)
      LOG(3, "watching %d with blit %d in red[%d][%d] %d %d %d %d%s", lit,
          other, (lidx & GLUEMASK), (lidx >> GLUESHFT), p[0], p[1], p[2], p[3],
          (p[4] ? " ..." : ""));
    else
      LOG(3, "watching %d with blit %d in irr[%d] %d %d %d %d%s", lit, other,
          lidx, p[0], p[1], p[2], p[3], (p[4] ? " ..." : ""));
  }
#endif
  assert(sizeof(res) == sizeof(void *));
  return res;
...
static void lglincocc(LGL *lgl, int lit) {
  ...
  LOG(3, "inc occ of %d now occs[%d] = %d %d", lit, idx, ev->occ[0],
      ev->occ[1]);
  if (ev->pos < 0)
    lglesched(lgl, idx);
  else if (change > 0)
...
static void lglrescoreclauses(LGL *lgl) {
  ...
  lglprt(lgl, 3, "[rescored-clauses-%d] rescored activity of %d clauses",
...
static int lglcheckgluereduced(LGL *lgl, int lidx, int gluelim, int sizelim,
...
static void lglcheckpromotion(LGL *lgl, int lidx) {
  ...
  if (!lglcheckgluereduced(lgl, lidx, gluelim, INT_MAX, 0))
    return;
  LOG(4, "saving clause idx red[%d][%d] on promotion stack", glue,
      (lidx >> GLUESHFT));
...
static int lgladdcls(LGL *lgl, int red, int origlue, int force) {
  ...
  if (red) {
    ...
    if (scaledglue == MAXGLUE && lgl->opts->keepmaxglue.val &&
      ...
      LOG(2, "reducing maximum glue %d to %d to keep clause", MAXGLUE,
          scaledglue);
    }
...
static void lglreset(LGL *lgl) {
  ...
  if (lgl->state <= USED)
    return;
  assert(lgl->state &
         (UNKNOWN | SATISFIED | EXTENDED | UNSATISFIED | FAILED | LOOKED));
  if (lgl->level > 0)
    lglbacktrack(lgl, 0);
...
static void lglbonflict(LGL *lgl, int lit, int blit) {
  ...
  lgl->conf.lit = lit;
  lgl->conf.rsn[0] = blit;
  LOG(2, "inconsistent %s binary clause %d %d", lglred2str(blit & REDCS), lit,
      (blit >> RMSHFT));
}
...
static void lgltonflict(LGL *lgl, int lit, int blit, int other2) {
  ...
  LOG(2, "inconsistent %s ternary clause %d %d %d", lglred2str(blit & REDCS),
      lit, (blit >> RMSHFT), other2);
}
...
static void lglrmtwch(LGL *lgl, int lit, int other1, int other2, int red) {
  ...
  HTS *hts;
  assert(!red || red == REDCS);
  LOG(3, "removing %s ternary watch %d blits %d %d", lglred2str(red), lit,
      other1, other2);
  hts = lglhts(lgl, lit);
  assert(hts->count >= 2);
...
static void lglrmlwch(LGL *lgl, int lit, int red, int lidx) {
  ...
  if (red)
    LOG(3, "removing watch %d in red[%d][%d] %d %d %d %d%s", lit,
        (lidx & GLUEMASK), (lidx >> GLUESHFT), p[0], p[1], p[2], p[3],
        (p[4] ? " ..." : ""));
  else
    LOG(3, "removing watch %d in irr[%d] %d %d %d %d%s", lit, lidx, p[0], p[1],
        p[2], p[3], (p[4] ? " ..." : ""));
#endif
  hts = lglhts(lgl, lit);
  assert(hts->count >= 2);
    ...
    if (tag == BINCS) {
      ...
      *q++ = other2;
      ...
      if (lgl->level && lgl->simp && lgl->opts->lhbr.val) {
        ...
        LOG(2, "hyper binary resolved %s clause %d %d", lglred2str(hbred), -dom,
            other2);
        lgldrupligaddclsarg(lgl, REDCS, -dom, other2, 0);
        if (subsumed) {
          LOG(2, "subsumes %s ternary clause %d %d %d", lglred2str(red), -lit,
              other, other2);
        ...
        if (dom == lit) {
          LOG(3,
              "replacing %s ternary watch %d blits %d %d with binary %d blit "
              "%d",
          ...
          if (dom == -other) {
            LOG(3, "removing %s ternary watch %d blits %d %d", lglred2str(red),
                -lit, other, other2);
            assert(subsumed);
            q -= 2;
          } else {
      ...
      if (lgl->level && lgl->simp && lgl->opts->lhbr.val) {
        ...
        if (dom == lit) {
          assert(subsumed);
          LOG(3, "replacing %s large watch %d with binary watch %d blit %d",
...
static int lglhasconflict(LGL *lgl) { return lgl->conf.lit != 0; }
...
static int lglbcpcomplete(LGL *lgl) {
  ...
  if (lgl->next2 < lgl->next)
    return 0;
  if (lgl->next < lglcntstk(&lgl->trail))
    return 0;
  return 1;
}
...
static int lglbcpsearch(LGL *lgl) {
  ...
  assert(!lgl->simp);
  assert(!lgl->notfullyconnected);
  while ((!lgl->failed || !lgl->level) && !lgl->conf.lit &&
...
static int lglpull(LGL *lgl, int lit) {
  ...
  if (level == lgl->level) {
    ...
    LOG(2, "adding literal %d at upper level %d to 1st UIP clause", lit,
        lglevel(lgl, lit));
...
static double lglavg(double n, double d) { return d != 0 ? n / d : 0.0; }
...
static double lglitder(LGL *lgl) {
  return 10000.0 *
         lglavg(lgl->stats->its.avg.diff.smoothed.val / (double)(1ull << 32),
                lgl->stats->confs);
}
...
static void lglrep(LGL *lgl, int level, char type) {
  ...
  lglprt(lgl, level,
         ...
         " %6.0f" // trail
         ,
         type, lgl->opts->abstime.val ? lglgetime(lgl) : lglsec(lgl),
         lglrem(lgl), lgl->stats->irr.clauses.cur, (LGLL)lgl->stats->confs,
         lgl->stats->red.lrg, lgl->stats->red.trn, lgl->stats->red.bin,
         lglglue(lgl), lglheight(lgl), lglitder(lgl), lgljlevelder(lgl),
         lglmb(lgl), 100.0 * (lgl->stats->agility / (double)(1ll << 32)),
...
static void lglfitlir(LGL *lgl, Stk *lir) { lglfitstk(lgl, lir); }
...
static void lglretire(LGL *lgl) {
  ...
  lglprt(lgl, 2, "[reduce-%d] %d ready to retire %.0f%%, %d inactive %.0f%%",
         lgl->stats->reduced.count, ready, lglpcnt(ready, count), inactive,
...
static void lglreduce(LGL *lgl, int forced) {
  ...
  if (target < 0)
    target = 0;
  lglprt(lgl, 2, "[reduce-%d] target is to collect %d clauses %.0f%%",
  ...
  for (i = 0; i < lglcntstk(&lgl->trail); i++) {
    ...
    LOGCLS(5, lir->start + lidx, "locking reason of literal %d glue %d clause",
           lit, glue);
#endif
    lidx /= 6;
    assert(maps);
  ...
  lglprt(lgl, 2, "[reduce-%d] locked %d learned reason clauses %.0f%%",
         lgl->stats->reduced.count, nlocked,
  ...
  lglprt(lgl, 2, "[reduce-%d] locked %d promoted learned clauses %.0f%%",
         lgl->stats->reduced.count, npromoted,
    ...
    for (c = start; c < lir->top; c = p + 1) {
      ...
      for (p = c; (lit = *p); p++)
        if (lgliphase(lgl, lit) >= 0)
          psm++;
      size = p - c;
      LOG(5, "clause red[%d][%d] has: psm %d, act %d, size %d", glue, lidx, psm,
          act, size);
  ...
  assert(npagsls <= szpagsls);
  LOG(2, "retiring %d learned clauses %.0f%%", retired,
      lglpcnt(retired, lgl->stats->red.lrg));
  ...
  for (idx = target; idx < npagsls; idx++) {
    ...
    maps[glue][lidx / 6] = -1;
    nkeep++;
    LOG(5, "marked clause red[%d][%d] to keep with psm %d, act %d, size %d",
    ...
    for (c = start; c < lir->top; c = p + 1) {
      ...
      if (map[src] <= -2) {
        ...
        if (p == q) {
          ...
          *q++ = act;
          LOGCLS(5, c, "moving from %d to %d glue %d clause", (c - start), dst,
                 glue);
          while (*p)
            *q++ = *p++;
          *q++ = 0;
  ...
  lglprt(lgl, 2,
         ...
         lgl->stats->reduced.count, sumcollected,
         lglpcnt(sumcollected, lgl->stats->red.lrg), sumretired,
  ...
  if ((lgl->opts->reducereset.val == 1 &&
    ...
    inc = lgl->opts->reduceinc.val;
    lglprt(lgl, 2, "[reduce-%d] increase of reduce limit by %d to %d",
...
static void lgliflushcache(LGL *lgl, int keep) {
  ...
  lglprt(lgl, 1, "[flush-cache] %d binary, %d ternary, %d large", bin, trn,
         lrg);
#ifndef NDEBUG
  lglclnstk(&lgl->prevclause);
  lgl->prevglue = -1;
...
static void lgldecocc(LGL *lgl, int lit) {
  ...
  if (!lglisfree(lgl, lit))
    return;
  change = lglecalc(lgl, ev);
  LOG(3, "dec occ of %d now occs[%d] = %d %d", lit, idx, ev->occ[0],
      ev->occ[1]);
  if (ev->pos < 0)
    lglesched(lgl, idx);
  else if (change < 0)
...
static void lgldynstr(LGL *lgl, int del, int lit, int r0, int r1) {
  ...
  tag = r0 & MASKCS;
  LOGREASON(2, lit, r0, r1, "on-the-fly strengthening by removing %d from",
            del);
  red = r0 & REDCS;
  if (red)
...
static void lglbumpdlit(LGL *lgl, int lit) {
  ...
  LOG(3, "bumping EVSIDS score of variable %d from %s to %s", idx,
      lglscr2str(lgl, oldscore), lglscr2str(lgl, newscore));
...
static void lglredcls(LGL *lgl, int uip, int glue, int *jlevelptr) {
  ...
  if (glue > lgl->opts->redclsglue.val)
    return;
  if (lglcntstk(&lgl->clause) >= lgl->opts->redclsize.val)
    return;
  ...
  if (lgl->opts->log.val >= 2) {
    ...
    for (p = lgl->clause.start; p < lgl->clause.top; p++) {
      if (!(lit = *p))
        continue;
      fprintf(lgl->out, " %d (l%d,t%d)", lit, lglevel(lgl, lit),
              lgltrail(lgl, lit));
    }
    lglogend(lgl);
  }
...
static int lglsubl(LGL *lgl, int lidx, int size, int glue) {
  ...
  LOGCLS(2, c, "subsumed learned (scaled) glue %d clause",
         (int)(lidx & GLUEMASK));
  LOGCLS(2, lgl->clause.start,
         "subsumed by new size %d glue %d scaled glue %d clause", size, glue,
...
static void lgldrive(LGL *lgl, const char *type, int *forcedptr, int *glueptr,
                     int *realglueptr, int *jlevelptr) {
  ...
  for (q = r = lgl->clause.start; q < lgl->clause.top; q++) {
    ...
    if (level > limlevel && p->used == 1) {
      ...
      *r++ = lit;
      ...
      LOG(4, "drop non-decision multi-used level %d literal %d in deco clause",
          level, lit);
  }
  lgl->clause.top = r;
...
static int lglvalidint64(int64_t a) { return a != INT64MIN && a != INT64MAX; }
...
#ifndef NLGLOG
  if (left)
    LOG(2, "EMA (left) input %lld old %g shift %d new %g", (LGLL)input,
        oldval / (double)(1ll << 32), ema->shift,
        ema->val / (double)(1ll << 32));
  else
    LOG(2, "EMA (right) input %g old %g shift %d new %g",
        input / (double)(1ll << 32), oldval / (double)(1ll << 32), ema->shift,
        ema->val / (double)(1ll << 32));
#else
    ;
#endif
...
static void lglupdatedema(LGL *lgl, DEMA *dema, int input) {
  ...
  dema->val = newval;
DONE:
  LOG(2, "DEMA input %lld old %g shift %d new %g", (LGLL)input,
      oldval / (double)(1ll << 32), dema->ema[0].shift,
      dema->val / (double)(1ll << 32));
}

#endif

static void lglinitmacd(LGL *lgl, MACD *macd, int fast, int slow, int smooth) {
...
DONE:
  LOG(2, "updated MACD input %lld actual/smoothed: old %g/%g, new %g/%g",
      (LGLL)input, oldactual / (double)(1ll << 32),
...
static void lglupdstab(LGL *lgl) {
  ...
  lglprt(lgl, 2,
         ...
         (LGLL)lgl->stats->stability.count, assigned, nontopassigned,
         lglpcnt(nontopassigned, assigned), changed,
  ...
  lgl->stats->stability.level = 0;
  lglprt(
      lgl, 2, "[stability-%lld] unchanged fraction %.3f%% new stability %.3f%%",
      (LGLL)lgl->stats->stability.count, 100.0 * fraction / (double)(1ll << 32),
      100.0 * lgl->stats->stability.avg.val / (double)(1ll << 32));
}

static void lglinitavg(AVG *avg) { avg->val = avg->count = 0; }
...
static void lglupdateavg(LGL *lgl, AVG *avg, int input) {
  ...
  avg->val = newval;
DONE:
  LOG(2, "AVG input %d old %g new %g count %lld", input,
      oldval / (double)(1ll << 32), newval / (double)(1ll << 32), avg->count);
}
...
static int lglblockrestart(LGL *lgl) {
  ...
  LOG(2, "%s restart MACD %g * %.2f = %g %s %g at %lld conflicts",
      (blocking ? "blocking" : "not blocking"), smoothed / (double)(1ll << 32),
      lgl->opts->restartblocklim.val / 100.0, limit / (double)(1ll << 32),
      blocking ? "<" : ">=", actual / (double)(1ll << 32),
      (LGLL)lgl->stats->confs);
  return blocking;
}
...
RESTART:
  ...
  for (;;) {
    ...
    LOG(3, "open %d antecendents %d learned %d resolved %d", open - 1, size,
        lglcntstk(&lgl->clause), lglcntstk(&lgl->resolvent));
    assert(open > 0);
    ...
    if (lgl->opts->otfs.val && (resolved >= 2) && resolventsize > 1 &&
      ...
      LOGREASON(2, lit, r0, r1, "driving %d at level %d through strengthened",
                lit, jlevel);
  ...
  if (origsize == 2) {
    ...
    LOGCLS(2, lgl->clause.start,
           "size %d 1st UIP clause with glue %d can not be minimized", origsize,
           glue);
  } else if (jlevel > 1 && lgl->opts->deco.val && redsize < 2 * origsize / 3 &&
  ...
  if (lgl->cbs && lgl->cbs->cls.produce.fun &&
    ...
    LOGCLS(2, lgl->eclause.start,
           "exporting external redundant glue %d length %d clause", glue,
           lglcntstk(&lgl->clause) - 1);
    lgl->stats->sync.cls.produced++;
    lgl->cbs->cls.produce.fun(lgl->cbs->cls.produce.state, lgl->eclause.start,
                              glue);
    lglclnstk(&lgl->eclause);
  }
...
static void lglincrestart(LGL *lgl) {
  ...
  if (lgl->opts->restartpen1.val && s->confs > 500000 &&
    ...
    lglprt(lgl, 2, "[restart-%lld] few iterations after 500000 conflicts",
           (LGLL)lgl->stats->restarts.count);
    pen++;
  }
  if (lgl->opts->restartpen2.val && s->confs > 50000 &&
      (!s->bins || s->confs / s->bins > 10000)) {
    lglprt(lgl, 2, "[restart-%lld] few binaries after 50000 conflicts",
           (LGLL)lgl->stats->restarts.count);
    pen++;
  }
  if (lgl->opts->restartpen3.val && s->confs > 5000 &&
      (!s->trns || s->confs / s->trns > 1000)) {
    lglprt(lgl, 2, "[restart-%lld] few ternaries after 5000 conflicts",
           (LGLL)lgl->stats->restarts.count);
    pen++;
  }
...
NOPEN:
  ...
  lglprt(lgl, 2, "[restart-%lld] next restart %lld delta %lld (penalty %d)",
         (LGLL)lgl->stats->restarts.count, (LGLL)lgl->limits->restart.confs,
         (LGLL)delta, pen);
  lglrep(lgl, 2, 'R');
}
...
static int lglnextdecision(LGL *lgl, int updatestats) {
  ...
  if (!res) {
    ...
    if (lglcntstk(&lgl->queue.stk) < 2 * lgl->queue.mt)
      lglqueueflush(lgl);
#ifndef NDEBUG
...
static int lglreusetrail(LGL *lgl) {
  ...
  if (res)
    lglprt(lgl, 3, "[reuse-trail] reusing level %d from current level %d", res,
           lgl->level);
  else
    lglprt(lgl, 3,
...
static int lglsetjwhbias(LGL *lgl, int idx) {
  ...
  if (bias > 0)
    ...
    lgl->stats->phase.neg++;
  LOG(1, "JW bias %d (pos %s, neg %s)", bias * idx, lglflt2str(lgl, pos),
      lglflt2str(lgl, neg));
  return bias;
}
...
static int lgluckyphase(LGL *lgl) {
  ...
  lglprt(lgl, 1, "[phase-count-%d] %d negative %.1f%% of %d except %d %.1f%%",
         lgl->stats->phase.count, neg, lglpcnt(neg, count), count, negrest,
         lglpcnt(negrest, count));
  lglprt(lgl, 1, "[phase-count-%d] %d positive %.1f%% of %d except %d %.1f%%",
         lgl->stats->phase.count, pos, lglpcnt(pos, count), count, posrest,
         lglpcnt(posrest, count));
  if (neg >= count) {
    res = -1;
    lglprt(lgl, 1, "[phase-count-%d] all clauses contain a negative literal",
           lgl->stats->phase.count);
  } else if (pos >= count) {
    res = 1;
    lglprt(lgl, 1, "[phase-count-%d] all clauses contain a positive literal",
           lgl->stats->phase.count);
    ...
    if (negrest <= lim && negrest <= posrest &&
      ...
      res = -1;
      lglprt(lgl, 1,
             "[phase-count-%d] less non-negative clauses %d than limit %d "
             "(%.1f%%)",
             lgl->stats->phase.count, negrest, lim,
             lgl->opts->phaselucklim.val / 10.0);
      lglprt(lgl, 1,
             "[phase-count-%d] non-pos/non-neg = %d/%d = %.2f >= %.2f = %d/100",
             lgl->stats->phase.count, posrest, negrest,
             posrest / (double)negrest, fac / 100.0, fac);
    } else if (posrest <= lim && posrest <= negrest &&
               negrest / fac >= posrest / 100) {
      res = 1;
      lglprt(lgl, 1,
             "[phase-count-%d] less non-positive clauses %d than limit %d "
             "(%.1f%%)",
             lgl->stats->phase.count, posrest, lim,
             lgl->opts->phaselucklim.val / 1000.0);
      lglprt(lgl, 1,
             "[phase-count-%d] non-neg/non-pos = %d/%d = %.2f >= %.2f = %d/100",
             lgl->stats->phase.count, negrest, posrest,
             negrest / (double)posrest, fac / 100.0, fac);
    }
  } else
    lglprt(lgl, 1, "[phase-count-%d] skipping relative phase luck test",
           lgl->stats->phase.count);
  if (res)
    lglprt(lgl, 1, "[lucky-phase-%d] forcing lucky phase %d",
           lgl->stats->phase.count, res);
  return res;
}
...
static void lglphase(LGL *lgl) {
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->phase.count, (LGLL)pos, lglpcnt(pos, set), (LGLL)neg,
         lglpcnt(neg, set));
DONE:
  lglstop(lgl);
}

static int lglmaplit(int *map, int lit) { return map[abs(lit)] * lglsgn(lit); }
...
static int lglsyncls(LGL *lgl) {
  ...
  if (lgl->stats->sync.cls.consumed.calls) {
    ...
    LOG(2, "syncls tried/calls = %lld/%lld = %d%%",
        ...
        (LGLL)lgl->stats->sync.cls.consumed.calls, rate);
    if (rate)
      delta /= rate;
  }
...
static void lglsimpleprobeinit(LGL *lgl) {
  ...
  if (!ret)
    ...
    lglprt(lgl, 1, "[simpleprobe-%d] %d schedulable variables %.0f%%",
...
static void lglsimpleprobeclscp(LGL *lgl, int nonrepr, int repr, Stk *stk) {
  ...
  LOG(2, "copied %d clauses with %d replacing %d by %d", count, nonrepr,
      nonrepr, repr);
}
...
static void lglsimpleliftmark(LGL *lgl, int root, int level) {
  ...
  for (p = w; p < eow; p++) {
    ...
    LOG(4, "next simple lifting marking from %d at level %d marked %d", root,
        level, other);
    if (level > 1)
...
static void lglprtsimpleproberem(LGL *lgl) {
  ...
  if (rem) {
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->prb.simple.count, rem, lglpcnt(rem, sum), ret,
           lglpcnt(ret, sum));
  } else {
...
static void lglsetprbsimplelim(LGL *lgl) {
  ...
  if (lgl->opts->prbsimplertc.val || lgl->opts->prbrtc.val) {
    ...
    lglprt(lgl, 1, "[simpleprobe-%d] really no limit (run to completion)",
           lgl->stats->prb.simple.count);
  } else {
    ...
    if (lgl->opts->prbsimplemaxeff.val >= 0 &&
      ...
      limit = lgl->opts->prbsimplemaxeff.val;
    if (lgl->stats->prb.simple.count <= 1 && lgl->opts->boost.val &&
        lgl->opts->prbsimpleboost.val > 1) {
      lglprt(lgl, 1, "[simpleprobe-%d] boosting simple probing limit by %d",
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1,
             ...
             lgl->stats->irr.clauses.cur, szpen);
    } else
      lglprt(lgl, 1, "[simpleprobe-%d] limit %lld penalty %d = %d + %d",
             lgl->stats->prb.simple.count, (LGLL)limit, pen,
             lgl->limits->prb.simple.pen, szpen);
...
static int lgldelaying(LGL *lgl, const char *name, int *delptr) {
  if (!*delptr)
    return 0;
  *delptr -= 1;
  lglprt(lgl, 2, "[%s-delayed] skipping this scheduled invocation and %d more",
         name, *delptr);
  return 1;
}
...
static int lglwaiting(LGL *lgl, const char *str, int wait) {
  ...
  if (wait >= 2 && lgl->opts->elim.val && !lgl->elmrtc) {
    if (str)
      lglprt(lgl, 2, "[%s-waiting] on variable elimination to be completed",
             str);
    return 1;
  }
...
static void lgltlsched(LGL *lgl) {
  ...
  for (p = lgl->tlk->seen.start; p < lgl->tlk->seen.top; p++)
    if (*p)
      count++;
  lglprt(lgl, 1, "[treelook-%d] scheduled %d literals %.0f%%",
         lgl->stats->prb.treelook.count, count,
...
static void lglincreducedptr(LGL *lgl, Flt *reducedptr, int size) {
  ...
  LOG(2, "reduced score incremented to %s from %s by %s (size %d)",
      lglflt2str(lgl, reduced), lglflt2str(lgl, *reducedptr),
      lglflt2str(lgl, inc), size);
  *reducedptr = reduced;
}

static int lgltlbcp(LGL *lgl, Stk *assignment, Flt *reducedptr, int dom) {
...
static void lgltreelooklit(LGL *lgl, Stk *a, Ftk *r, int lit) {
  ...
  if (lit) {
    ...
    if (lgltlval(lgl, -lit) > 0 ||
        (!lgltlval(lgl, lit) && !lgltlbcp(lgl, a, r ? &reduced : 0, lit))) {
      int tmp;
...
static int64_t lglsetprbtreelooklim(LGL *lgl, int *lkhdresptr) {
  ...
  if (lgl->opts->treelookrtc.val || lgl->opts->prbrtc.val) {
    ...
    lglprt(lgl, 1, "[treelook-%d] really no limit (run to completion)",
    ...
    szpen = lglszpen(lgl);
    lastpen =
        lgl->tlk->lkhd ? lgl->limits->lkhdpen : lgl->limits->prb.treelook.pen;
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[treelook-%d] limit %lld penalty %d = %d + %d boost %d",
             lgl->stats->prb.treelook.count, (LGLL)limit, pen, lastpen, szpen,
             boost);
    if (lkhdresptr)
...
static void lgltlsetlkhd(LGL *lgl, int *lkhdresptr, int remlits) {
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
    ...
    LOG(1,
        ...
        lglflt2str(lgl, lkhdneg), lglflt2str(lgl, jwhneg), idx);
...
static void lglgenericreport(LGL *lgl, const char *name, int count, int probed,
                             int nprobes, double start) {
  int div, mod;
  assert(nprobes >= 0);
  ...
  if (probed % mod)
    return;
  lglprt(lgl, 1, "[%s-%d] probed %d/%d (%.0f%%) %.1f sec %d vars %.0f MB", name,
         count, probed, nprobes, lglpcnt(probed, nprobes),
         lglgetime(lgl) - start, lglrem(lgl), lglmb(lgl));
}

static void lgltreelookreport(LGL *lgl, int probed, int nprobes, double start) {
  lglgenericreport(lgl, "treelook", lgl->stats->prb.treelook.count, probed,
                   nprobes, start);
}
...
static int lgltreelookaux(LGL *lgl, int *lkhdresptr) {
  ...
  if (!lgl->mt && nseen) {
    ...
    while (!lgl->mt && !lglterminate(lgl) && lglsyncunits(lgl) &&
           next < nseen &&
           lgl->stats->prb.treelook.steps < lgl->limits->prb.steps) {
  ...
  if (!lkhdresptr)
    ...
    lgl->limits->lkhdpen--;
  lglprt(lgl, 1 + !success, "[treelook-%d] removed %d variables, found %d hbrs",
...
static int lgljwhlook(LGL *lgl) {
  ...
  if (res) {
    ...
    ext = lglelit2ext(lgl, elit);
    lglprt(lgl, 1, "[jwhlook] best look-ahead %d score %s", res,
           lglflt2str(lgl, best));
    if (ext->melted) {
      ext->melted = 0;
...
static int lglislook(LGL *lgl) {
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
    ...
    if (res && score <= best)
      continue;
    LOG(1, "LIS look-ahead score %lld (pos %lld, neg %lld) of %d", (LGLL)score,
        (LGLL)pos, (LGLL)neg, idx);
    res = (pos > neg) ? idx : -idx;
    best = score;
  }
  scores -= lgl->nvars;
  ...
  if (res) {
    ...
    lglprt(lgl, 1, "[lislook] best LIS look-ahead %d score %lld", res,
           (LGLL)best);
    if (ext->melted) {
      ext->melted = 0;
...
static int lglsumlenlook(LGL *lgl) {
  ...
  for (idx = 2; idx < lgl->nvars; idx++) {
    ...
    if (res && score <= best)
      continue;
    LOG(1, "look-ahead score %lld (pos %lld, neg %lld) of %d", (LGLL)score,
        (LGLL)pos, (LGLL)neg, idx);
    res = (pos > neg) ? idx : -idx;
    best = score;
  }
...
static int lglschedbasicprobe(LGL *lgl, Stk *probes, int round) {
  ...
  if (!res)
    ...
    lglprt(lgl, 2, "[basicprobe-%d-%d] scheduled %d probes %.0f%%",
           lgl->stats->prb.basic.count, round, res, lglpcnt(res, lglrem(lgl)));
  return res;
}
...
static void lglsetprbasiclim(LGL *lgl) {
  ...
  if (lgl->opts->prbasicrtc.val || lgl->opts->prbrtc.val) {
    lgl->limits->prb.steps = LLMAX;
    lglprt(lgl, 1, "[basicprobe-%d] really no limit (run to completion)",
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[basicprobe-%d] limit %lld penalty %d = %d + %d",
             lgl->stats->prb.basic.count, (LGLL)limit, pen,
             lgl->limits->prb.basic.pen, szpen);
    lgl->limits->prb.steps = lgl->stats->prb.basic.steps + limit;
  }
}

static void lglbasicprobereport(LGL *lgl, int probed, int nprobes,
                                double start) {
  lglgenericreport(lgl, "basicprobe", lgl->stats->prb.basic.count, probed,
                   nprobes, start);
}
  ...
  while (!lgl->mt) {
    ...
    if (!root || root == first) {
      lglprt(
          ...
          lgl->stats->prb.basic.count, round, nprobes,
  ...
  if (!lgl->mt) {
    if (lgl->stats->prb.basic.steps >= lgl->limits->prb.steps) {
      ...
      lglprt(lgl, 1,
             ...
             lgl->stats->prb.basic.count, round, remprobes,
             lglpcnt(remprobes, lglrem(lgl)));
    } else if (round >= lgl->opts->prbasicroundlim.val) {
      lglprt(lgl, 1, "[basicprobe-%d-%d] round limit %d hit",
             lgl->stats->prb.basic.count, round,
      ...
      assert(!remprobes);
      lglprt(lgl, 1, "[basicprobe-%d-%d] fully completed probing",
             lgl->stats->prb.basic.count, round);
      for (idx = 2; idx < lgl->nvars; idx++)
        lglavar(lgl, idx)->donotbasicprobe = 0;
    }
...
DONE:
  ...
  LOG(1, "found %d failed %.1f%% lifted %d through probing", failed,
      lglpcnt(failed, probed), lifted);
#endif
  ...
  lglprt(
      ...
      lgl->stats->prb.basic.count, round, units, failed, lglpcnt(failed, units),
      lifted, lglpcnt(lifted, units));
  lglprt(lgl, 1 + !success,
  ...
  if (lgl->occs && lgl->opts->verbose.val >= 1) {
    ...
    for (idx = 2; idx < lgl->nvars; idx++)
      if (lglevar(lgl, idx)->pos >= 0)
        count++;
    lglprt(lgl, vl, "[%s-%d] scheduled %d variables %.0f%%", str, inst, count,
           lglpcnt(count, lgl->nvars - 2));
  }
}
...
static int lglbacksub(LGL *lgl, int *c, int str, int **dptr, int *dlidxptr) {
  ...
  for (next = lglpeek(&lgl->elm->occs, ulit);
       !res && next && lgl->limits->elm.steps > lgl->stats->elm.steps;
...
static void lglelmsub(LGL *lgl) {
  ...
  for (c = lgl->elm->lits.start + 1;
       c < lgl->elm->lits.top && lgl->limits->elm.steps > lgl->stats->elm.steps;
       c++) {
    INCSTEPS(elm.steps);
  ...
  LOG(BWL + !subsumed, "subsumed %d clauses containing %d or %d", subsumed,
      lgl->elm->pivot, -lgl->elm->pivot);
}
...
static int lglelmstr(LGL *lgl) {
  ...
  LOG(2, "strengthened %d clauses containing %d or %d", strengthened,
      lgl->elm->pivot, -lgl->elm->pivot);
  return res;
}
...
static int lglflushclauses(LGL *lgl, int lit) {
  ...
  for (p = w; p < eow; p++) {
    ...
    if (tag == BINCS) {
      ...
      LOG(2, "flushed %s ternary clause %d %d %d", lglred2str(red), lit, other,
          other2);
      if (druplig)
        lgldrupligdelclsarg(lgl, lit, other, other2, 0);
      lgldeclscnt(lgl, 3, red, 0);
...
#endif
  lglshrinkhts(lgl, hts, 0);
  LOG(2, "flushed %d clauses with %d including %d irredundant", count, lit,
      res);
  lglchkirrstats(lgl);
  return res;
}
...
#define FIXPTRS()                                                              \
  ...
  for (p = w; p < eow; p++) {
    ...
    if (tag == BINCS) {
      ...
      if (!val && !val2) {
        LOG(2,
            "reducing flushed %s ternary clause %d %d %d to binary %s clause "
            "%d %d",
            lglred2str(red), lit, other, other2, lglred2str(red), other,
            other2);
  ...
  hts = lglhts(lgl, lit);
  lglshrinkhts(lgl, hts, 0);
  LOG(2, "flushed %d occurrences of literal %d including %d irredundant", count,
      lit, res);
  lglchkirrstats(lgl);
  return res;
...
static int lglflush(LGL *lgl) {
  ...
  assert(!lgl->level);
  assert(lgl->probing || lgl->lkhd || lgl->dense);
  if (lgl->flushed == lglcntstk(&lgl->trail))
    return 1;
  if (!lglbcp(lgl)) {
    lglmt(lgl);
    return 0;
...
static void lglelmfrelit(LGL *lgl, int mpivot, int *sop, int *eop, int *son,
                         int *eon) {
  ...
  assert(ipivot);
  LOG(3, "blocked clause elimination and forced resolution of clauses with %d",
      ipivot);
  clidx = 0;
  ...
  for (c = sop; c < eop && limit >= 0; c = p + 1) {
    ...
    for (d = son; limit >= 0 && d < eon; d = q + 1) {
      ...
      if (!lit && reslen == 1) {
        ...
        limit--;
        LOG(3, "trying resolution with non trivial resolvent remaining %d",
            limit);
        if (reslen > maxreslen)
          maxreslen = reslen;
      }
...
static void lglvar2fun(int v, Fun res) { lglvar2funaux(v, res, 0); }

static void lglnegvar2fun(int v, Fun res) { lglvar2funaux(v, res, 1); }
...
static Cnf lglpos2cnf(int pos) {
  assert(pos >= 0);
  return pos;
}
static Cnf lglsize2cnf(int s) {
  assert(s >= 0);
  return ((Cnf)s) << 32;
}
...
static int lglsmalltopvar(const Fun f, int min) {
  ...
  switch (min) {
  case 0:
    if (lglsmallfundeps0(f))
      return 0;
  case 1:
    if (lglsmallfundeps1(f))
      return 1;
  case 2:
    if (lglsmallfundeps2(f))
      return 2;
  case 3:
    if (lglsmallfundeps3(f))
      return 3;
  case 4:
    if (lglsmallfundeps4(f))
      return 4;
  case 5:
    if (lglsmallfundeps5(f))
      return 5;
  }
...
static int lgltrysmallve(LGL *lgl, int idx) {
  ...
  if (lglinitsmallve(lgl, idx, pos) && lglinitsmallve(lgl, -idx, neg)) {
    ...
    LOG(2,
        "small elimination of %d replaces %d old with %d new clauses and %d "
        "units",
        idx, old, new, units);
    ...
    if (new > old) {
      if (units > 0) {
        LOG(2,
            "unforced small elimination of %d would add %d clauses but "
            "produces %d units",
            idx, new - old, units);
        res = 1;
      } else {
        LOG(2,
            "unforced small elimination of %d would add %d clauses considered "
            "failed",
            idx, new - old);
      ...
      LOG(2,
          "small elimination of %d removes %d clauses thus considered "
          "promising",
          idx, old - new);
...
static void lglsetblklim(LGL *lgl) {
  ...
  if (lgl->opts->blkrtc.val) {
    ...
    lglprt(lgl, 1, "[block-%d] really no limit (run to completion)",
           lgl->stats->blk.count);
  } else {
    ...
    if (lgl->stats->blk.count <= 1 && lgl->opts->boost.val &&
        lglrem(lgl) < lgl->opts->blkboostvlim.val) {
      lglprt(lgl, 1, "[block-%d] boosting limit by %d", lgl->stats->blk.count,
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[block-%d] limit of %lld steps penalty %d = %d + %d",
             lgl->stats->blk.count, (LGLL)limit, pen, lgl->limits->blk.pen,
             szpen);
...
static int lgleschedrem(LGL *lgl, int this_time) {
  ...
  if (res)
    lglprt(lgl, 1, "[%s-%d] %d variables %.0f%% %s time", str, count, res,
...
static void lglblock(LGL *lgl) {
  ...
  all = !oldrem || !oldall;
  if (all)
    lglprt(lgl, 1, "[block-%d] scheduling all variables this time",
           lgl->stats->blk.count);
  ...
  if (!rem) {
    ...
    lglprt(lgl, 1, "[block-%d] incomplete %d not tried %.0f%%",
  ...
  if (!lgl->blkrtc &&
    ...
    if (!success)
      lglprt(lgl, 1, "[block-%d] %d < 1/%d * %d = %d considered unsuccessful",
             lgl->stats->blk.count, count, lgl->opts->blksuccessrat.val, oldirr,
...
static void lglsetccelim(LGL *lgl) {
  ...
  if (lgl->opts->ccertc.val > 1) {
    ...
    lglprt(lgl, 1, "[cce-%d] really no limit (run to completion)",
           lgl->stats->cce.count);
  } else if (lgl->opts->ccertc.val ||
             (count > 0 && lglrem(lgl) < lgl->opts->ccertcintvlim.val &&
              !(count % lgl->opts->ccertcint.val))) {
    limit = 4000000000ll;
    lgl->limits->cce.steps = lgl->stats->cce.steps + limit;
    lglprt(lgl, 1, "[cce-%d] almost no limit of %lld steps",
    ...
    if (count > 0 && (count <= 1 || !(count % lgl->opts->cceboostint.val)) &&
        lgl->opts->boost.val && lgl->nvars < lgl->opts->cceboostvlim.val) {
      lglprt(lgl, 1, "[cce-%d] boosting limit by %d", lgl->stats->cce.count,
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[cce-%d] limit of %lld steps penalty %d = %d + %d",
             lgl->stats->cce.count, (LGLL)limit, pen, lgl->limits->cce.pen,
             szpen);
    lgl->limits->cce.steps = lgl->stats->cce.steps + limit;
  }
}
...
static int lglcceclause(LGL *lgl, const int *c, int igntag, int cce) {
...
ALA:
      ...
      *newtop++ = blit;
      ...
      if (tag == BINCS) {
        if (lglsignedmarked(lgl, other)) {
          ...
          LOG(CCELOGLEVEL, "ALA on binary clause %d %d results in ATE", lit,
              other);
          res = 1;
        } else {
          assert(!lglmarked(lgl, other));
          LOG(CCELOGLEVEL, "ALA %d through binary clause %d %d", -other, lit,
              other);
          lglsignedmark(lgl, -other);
          lglpushstk(lgl, &lgl->seen, -other);
        }
        ...
        if (lglsignedmarked(lgl, other)) {
          if (lglsignedmarked(lgl, other2)) {
            ...
            LOG(CCELOGLEVEL, "ALA on ternary clause %d %d %d results in ATE",
                lit, other, other2);
            res = 1;
          } else {
            assert(!lglmarked(lgl, other2));
            LOG(CCELOGLEVEL,
                "ALA %d through ternary clause %d %d %d (1st case)", -other2,
                lit, other, other2);
          ...
          assert(!lglmarked(lgl, other));
          LOG(CCELOGLEVEL, "ALA %d through ternary clause %d %d %d (2nd case)",
  ...
  while (!res && nextcla < lglcntstk(&lgl->cce->cla)) {
    ...
    if (!res && p == eow && nextala < lglcntstk(&lgl->seen))
      goto ALA;
  }
SKIPCLA:
  if (res) {
...
static void lglccesmall(LGL *lgl, int cce, int round) {
  ...
  lglprt(lgl, 1, "[cce-%d-%d] scheduling %d clauses = %d binary + %d ternary",
  ...
  if (tried)
    lglprt(lgl, 1, "[cce-%d-%d] tried to eliminate %d small clauses %.0f%%",
           lgl->stats->cce.count, round, tried, lglpcnt(tried, count));
  if (elim2)
    lglprt(lgl, 1, "[cce-%d-%d] eliminated %d binary clauses %.0f%%",
           lgl->stats->cce.count, round, elim2, lglpcnt(elim2, lgl->cce->bin));
  if (elim3)
    lglprt(lgl, 1, "[cce-%d-%d] eliminated %d ternary clauses %.0f%%",
           lgl->stats->cce.count, round, elim3, lglpcnt(elim3, lgl->cce->trn));
  if (eliminated)
    lglprt(lgl, 1, "[cce-%d-%d] eliminated %d small clauses %.0f%%",
           lgl->stats->cce.count, round, eliminated,
...
RESTART:
  ...
  if (cce == 3 && lgl->ccertc < 2 &&
    ...
    lglprt(lgl, 2, "[cce-%d-%d] restricted to ABCE since ccertc=%s",
           lgl->stats->cce.count, round, lglcce2str(lgl->ccertc));
    cce = 2;
  }
  if (cce == 2 && lgl->ccertc < 1 &&
      lgl->opts->cce2wait.val >= lgl->stats->cce.count) {
    lglprt(lgl, 2, "[cce-%d-%d] restricted to ATE since ccertc=%s",
           lgl->stats->cce.count, round, lglcce2str(lgl->ccertc));
    cce = 1;
  }
  if (cce > 1 && !(lgl->stats->cce.count % lgl->opts->cceateint.val)) {
    lglprt(lgl, 2, "[cce-%d-%d] restricted to ATE due to ATE interval %d",
           lgl->stats->cce.count, round, lgl->opts->cceateint.val);
    cce = 1;
  }
  oldirr = lgl->stats->irr.clauses.cur;
  lglprt(lgl, 2, "[cce-%d-%d] starting round %d (%s)", lgl->stats->cce.count,
         round, round, lglcce2str(cce));
  ...
  elmlarge = 0;
  while (!lgl->mt && lenlim > 4 && !lglterminate(lgl) &&
    ...
    lglprt(lgl, 2, "[cce-%d-%d] scheduling %d clauses of length %d",
           lgl->stats->cce.count, round, count, maxlen);
    eliminated = 0;
    for (p = lidcs.start; p < lidcs.top && !lgl->mt && !lglterminate(lgl) &&
                          lgl->limits->cce.steps > lgl->stats->cce.steps;
         p++) {
      int lidx = *p;
    ...
    lglprt(lgl, 1 + !eliminated,
           ...
           lgl->stats->cce.count, round, eliminated, count,
           lglpcnt(eliminated, count), maxlen);
  }
  completedlarge = (lgl->limits->cce.steps > lgl->stats->cce.steps);
  if (completedlarge)
    lglprt(lgl, 1, "[cce-%d-%d] completed large round (%s)",
           lgl->stats->cce.count, round, lglcce2str(cce));
  else
    lglprt(lgl, 1, "[cce-%d-%d] incomplete large round (%s)",
           lgl->stats->cce.count, round, lglcce2str(cce));
  ...
  if (LLMAX - deltasteps > lgl->limits->cce.steps) {
    ...
    lglprt(lgl, 1, "[cce-%d-%d] allowing another %lld steps for small clauses",
           lgl->stats->cce.count, round, deltasteps);
  } else {
    lgl->limits->cce.steps = LLMAX;
    lglprt(lgl, 1, "[cce-%d-%d] unlimited number of steps for small clauses",
  ...
  if (completedsmall)
    lglprt(lgl, 1, "[cce-%d-%d] completed small round (%s)",
           lgl->stats->cce.count, round, lglcce2str(cce));
  else
    lglprt(lgl, 1, "[cce-%d-%d] incomplete small round (%s)",
           lgl->stats->cce.count, round, lglcce2str(cce));
  if (completedsmall && completedlarge) {
    if (lgl->ccertc < cce) {
      lgl->ccertc = cce;
      lglprt(lgl, 1, "[cce-%d-%d] completed small and large (%s)",
             lgl->stats->cce.count, round, lglcce2str(cce));
    }
  }
  lglsparse(lgl);
  ...
  if (!lgl->mt && eliminated && !lglterminate(lgl) &&
  ...
  if (total) {
    ...
    if (!success)
      lglprt(lgl, 1, "[cce-%d] %d < 1/%d * %d = %d considered unsuccessful",
...
static void lglsetelmlim(LGL *lgl, int *reschedptr) {
  ...
  if (lgl->opts->elmrtc.val > 1) {
    ...
    if (count > 0 && (count == 1 || !(count % lgl->opts->elmboostint.val)) &&
        lglrem(lgl) < lgl->opts->elmboostvlim.val && lgl->opts->boost.val &&
        lgl->opts->elmboost.val > 1) {
      lglprt(lgl, 1, "[elim-%d] boosting limit by %d", lgl->stats->elm.count,
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[elim-%d] limit of %lld steps penalty %d = %d + %d",
             lgl->stats->elm.count, (LGLL)limit, pen, lgl->limits->elm.pen,
             szpen);
    lgl->limits->elm.steps = lgl->stats->elm.steps + limit;
  }
  lglprt(lgl, 1, // TODO 2?
...
static int lglforceschedall(LGL *lgl) {
  ...
  if (oldonotsched)
    lgl->donotsched = 1;
  lglprt(lgl, 1, "[elim-%d-%d] fully rescheduled %d variables %.0f%%",
         lgl->stats->elm.count, lgl->elm->round, res,
...
static int lglbackwardlit(LGL *lgl, const int *clause, const int *skip,
                          int size, int minlit, int *subptr, int *strptr) {
    ...
    if (tag == BINCS) {
      ...
      if (marked > 0) {
        ...
        *subptr += 1;
        ...
        LOG(BWL, "removing %d and thus strengthening binary clause %d %d",
            other, minlit, other);
        assert(p != skip);
        ADDSTEPS(elm.steps, 2);
        lglunit(lgl, minlit);
      ...
      if ((size == 2 && (marked > 0 || marked2 > 0)) ||
        ...
        *subptr += 1;
        ...
        if (marked < 0 && marked2 > 0) {
          ...
          continue;
        LOG(BWL, "removing %d and thus strengthening ternary clause %d %d %d",
...
static int lglbackwardclause(LGL *lgl, const int *clause, const int *skip,
  ...
  res = lglbackwardlit(lgl, clause, skip, size, minlit, subsumedptr,
                       strengthenedptr);
  if (!res) {
    res = lglbackwardlit(lgl, clause, skip, size, minlit2, subsumedptr,
                         strengthenedptr);
  }
  if (res && large)
...
static void lglbackward(LGL *lgl, int *u, int *t, int64_t steps,
  ...
  *u = *t = 0;
  ...
  lglprt(lgl, 1, "[elim-%d-%d] backward subsumption/strengthening limit %lld",
...
RESTARTOUCHED:
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->elm.count, lgl->elm->round, round, oldtouched,
...
DONE:
  ...
  if (*completedptr)
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->elm.count, lgl->elm->round, round, newtouched,
...
static int lglelmdone(LGL *lgl, int *allptr) {
  ...
  if (eliminated <= 0) {
    lglprt(
        lgl, 1, "[elim-%d-%d] no variable eliminated in round %d in %lld steps",
        lgl->stats->elm.count, lgl->elm->round, lgl->elm->round, (LGLL)steps);
    return 1;
  }
  lglprt(
      lgl, 1,
  ...
  if (!lglmtstk(&lgl->esched)) {
    lglprt(
        ...
        lgl->stats->elm.count, lgl->elm->round, lglcntstk(&lgl->esched),
...
static int lglelim(LGL *lgl) {
  ...
  all = !oldrem || !oldall;
  if (all)
    lglprt(lgl, 1, "[elim-%d] scheduling all variables this time",
           lgl->stats->elm.count);
  else if (!lgleschedrem(lgl, 1))
    all = 1, oldrem = 0;
  if (!all)
  ...
  rem = lglcntstk(&lgl->esched);
  if (!rem) {
    lglprt(lgl, 1, "[elim-%d] fully completed in %d rounds",
           lgl->stats->elm.count, lgl->elm->round);
    lgl->elmrtc = 1;
  } else {
    lglprt(lgl, 1, "[elim-%d] incomplete %d not tried %.0f%% in round %d",
  ...
  if (!lgl->elmrtc &&
    ...
    if (!success)
      lglprt(lgl, 1, "[elim-%d] %d < 1/%d * %d = %d considered unsuccessful",
             lgl->stats->elm.count, elmd, lgl->opts->elmsuccessrat.val, sched,
...
static int lglsynceqs(LGL *lgl) {
  ...
  for (elit1 = 1; elit1 <= emax; elit1++) {
    ...
    if (abs(irepr2) <= 1)
      continue;
    LOG(2, "importing external equivalence %d %d as internal %d %d", elit1,
        elit2, irepr1, irepr2);
...
DONE:
  ...
  if (removed)
    lglprt(lgl, 1, "[decomp-%d] removed %d variables", lgl->stats->decomps,
           removed);
  lglrep(lgl, 2, 'd');
  lglstop(lgl);
  return res;
...
static void lglanafailed(LGL *lgl) {
  ...
  if (lgl->mt) {
    ...
    if ((av = lglavar(lgl, failed))->assumed == 3) {
      ...
      assert(size > 0);
      lglpushstk(lgl, &lgl->clause, 0);
      lglprt(lgl, 2, "[analyze-final] learned clause with size %d out of %d",
    ...
    for (p = lgl->eassume.start; p < lgl->eassume.top; p++) {
      ...
      if (ext->equiv) {
        ...
        if (rext->assumed & rbit) {
          ...
          LOG(2, "found non representative external failed assumption %d",
              elit);
          ext->failed |= bit;
        }
      } else {
...
static void lglseternreslim(LGL *lgl) {
  ...
  if (lgl->opts->ternresrtc.val) {
    ...
    lglprt(lgl, 1, "[ternres-%d] really no limit (run to completion)",
    ...
    if (lgl->stats->ternres.count <= 1 && lgl->opts->boost.val &&
        lgl->opts->ternresboost.val > 1) {
      lglprt(lgl, 1, "[ternres-%d] boosting ternary resolution limit by %d",
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1, "[ternres-%d] limit %lld with penalty %d = %d + %d",
             lgl->stats->ternres.count, (LGLL)limit, pen,
             lgl->limits->ternres.pen, szpen);
...
static void lglprternresrem(LGL *lgl) {
  ...
  if (rem) {
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->ternres.count, rem, lglpcnt(rem, sum), ret,
           lglpcnt(ret, sum));
  } else {
...
static void lglternresinit(LGL *lgl) {
  ...
  if (!donoternres)
    ...
    lglprt(lgl, 1, "[ternres-%d] %d schedulable variables %.0f%%",
           lgl->stats->ternres.count, schedulable,
...
static int lglquatres1(LGL *lgl, int *trnptr) {
  ...
  for (glue = -1; glue <= maxglue; glue++) {
    ...
    if (glue < 0)
      ...
      lglprt(lgl, 2, "[quatres-%d] found %d glue %d quaternary clauses %.0f%%",
    ...
    for (j = i + 1; j < total; j++) {
      ...
      if (pivot) {
        ...
        if (lglhastrn(lgl, m[0], m[1], m[2]))
          continue;
        red = redi & redj;
        LOG(2, "self-subsuming %s ternary resolvent %d %d %d",
...
static void lglquatres2(LGL *lgl, int *trnptr, int *quadptr) {
          ...
          for (q = w; check < maxcheck && q < eow; q++) {
            ...
            if (maxglue < MAXGLUE && tag == LRGCS) {
              ...
              if (!lglhasquad(lgl)) {
                ...
                LOG(5, "before: w + %d = q = eow - %d", (int)(q - w),
                    (int)(eow - w));
                lgldrupligaddcls(lgl, REDCS);
                ...
                if (w + hts->count != eow) {
                  LOG(5, "fixing %d watches end from %x to %x", sign * lit, eow,
                      w + hts->count);
                  eow = w + hts->count;
                }
                LOG(5, "after: w + %d = q = eow - %d", (int)(q - w),
                    (int)(eow - w));
              ...
              LOGCLS(0, c, "1st pivot %d quaternary scaled glue %d antecedent",
                     lit, glue);
              LOG(0, "2nd pivot %d ternary antecedent %d %d %d", -lit, -lit,
                  other, other2);
              extra = 0;
              for (l = c; !extra; l++) {
                other3 = *l;
              ...
              LOG(0, "self-subsuming %s ternary resolvent %d %d %d",
  ...
  lglprt(lgl, 1 + !trn,
         ...
         lgl->stats->quatres.count, trn);
  lglprt(lgl, 1 + !quat, "[quatres-%d-2] added %d quaternary resolvents",
...
static void lgltrdlit(LGL *lgl, int start) {
  ...
  w = lglhts2wchs(lgl, hts);
  eow = w + hts->count;
  for (p = w; p < eow && (lgl->stats->trd.steps < lgl->limits->trd.steps);
       p++) {
    blit = *p;
    tag = blit & MASKCS;
    if (tag == TRNCS || tag == LRGCS)
...
static void lglsetrdlim(LGL *lgl) {
  ...
  if (lgl->opts->irrlim.val && limit < irrlim) {
    ...
    lglprt(
        ...
        lgl->stats->trd.count, (LGLL)limit, lgl->stats->irr.clauses.cur, szpen);
  } else
    lglprt(lgl, 1, "[transred-%d] limit %lld with penalty %d = %d + %d",
           lgl->stats->trd.count, (LGLL)limit, pen, lgl->limits->trd.pen,
           szpen);
...
static int lgltrd(LGL *lgl) {
  ...
  assert(lgl->simp);
  lglprt(lgl, 1, "[transred-%d] removed %d transitive binary clauses",
         lgl->stats->trd.count, red);
  lglprt(lgl, 1 + !failed, "[transred-%d] found %d units",
...
static int lglunhdhasbins(LGL *lgl, const DFPR *dfpr, int lit, int irronly) {
...
static void lglpushwtk(LGL *lgl, Wtk *wtk, Wrag wrag, int lit, int other,
                       int red) {
  Work w;
  if (lglfullwtk(wtk))
    lglenlwtk(lgl, wtk);
...
static int lglstamp(LGL *lgl, int root, DFPR *dfpr, DFOPF *dfopf, Wtk *work,
                    Stk *units, Stk *sccs, Stk *trds, int *visitedptr,
                    int stamp, int irronly) {
    ...
    if (wrag == PREFIX) {
      ...
      *visitedptr += 1;
      ...
      if (lgl->opts->unhdextstamp.val && startstamp <= observed) {
        ...
        for (failed = lit; dfpr[lglulit(failed)].discovered > observed;
        ...
        if (dfpr[unotother].discovered && !dfpr[unotother].finished) {
          LOG(2, "stamping skips edge %d %d after failed literal %d", lit,
              other, failed);
          work->top--;
          continue;
        }
      ...
      if (lgl->opts->unhdextstamp.val && !dfpr[uother].finished &&
        ...
        LOG(3, "stamping %d reduced discovered to %d", lit,
            dfpr[ulit].discovered);
        if (dfopf[ulit].flag) {
      ...
      if (dfopf[ulit].flag) {
        ...
        do {
          ...
          LOG(3, "stamping %d interval %d %d parent %d root %d", other,
              dfpr[uother].discovered, dfpr[uother].finished,
...
static int lglunhidebintrn(LGL *lgl, const DFPR *dfpr, int irronly) {
        ...
        *q++ = blit;
        ...
        if (tag == BINCS) {
          ...
          UNIT:
            ...
            LOG(2, "negated literals in binary clause %d %d implied by %d", lit,
                other, root);
            ...
            nbinred++;
            lglrmbwch(lgl, other, lit, red);
            LOG(2, "removed %s binary clause %d %d", lglred2str(red), lit,
                other);
            lgldeclscnt(lgl, 2, red, 0);
            assert(!lgl->dense);
            q--;
          ...
          if (lglunhimplies2incl(dfpr, other, lit) &&
            ...
            LOG(2,
                ...
                other, other2, other, lit, other2, lit, lit, other, other2);
            ...
            lglrmtwch(lgl, other, lit, other2, red);
            lglrmtwch(lgl, other2, lit, other, red);
            LOG(2, "removed %s ternary clause %d %d %d", lglred2str(red), lit,
                other, other2);
            lgldeclscnt(lgl, 3, red, 0);
            assert(!lgl->dense);
          ...
          TRNSTR:
            ...
            goto TRNSTR;
                     ...
                     (root = lglunhroot(dfpr, -lit)) && !lglval(lgl, root)) {
            ...
            LOG(2,
                ...
                "implied by %d",
                lit, other2, lit, other, other2, lca);
  ...
  if (ntrnstr)
    LOG(2, "unhidden %d hyper binary resolution from ternary clauses",
        ntrnhbrs);
  return 1;
}
...
static int lglunhideglue(LGL *lgl, const DFPR *dfpr, int glue, int irronly) {
  ...
  HTE:
    ...
    while (d < eodfl - 1) {
      ...
      for (e = d + 1; e < eodfl && e->finished < d->finished; e++) {
        ...
        LOGCLS(2, c, "unhiding with implication %d %d tautological %s clause",
  ...
  HBR:
    ...
    if (lglunhimplies2incl(dfpr, root1, -root2))
      goto NEXT;
    LOGCLS(2, c, "root hyper binary resolution %d %d of %s clause", -root1,
           -root2, type);
    if (!lca1 && !lca2) {
      for (p = c; (lit = *p); p++) {
        root = lglunhroot(dfpr, -lit);
    ...
    if (lglunhimplies2incl(dfpr, lca1, -lca2))
      goto NEXT;
    LOGCLS(2, c, "lca hyper binary resolution %d %d of %s clause", -lca1, -lca2,
           type);
...
static DFPR *lglstampall(LGL *lgl, int irronly) {
  ...
  for (rootsonly = 1; rootsonly >= 0; rootsonly--) {
    ...
    for (;;) {
      ...
      searches++;
      assert(lglmtstk(&units));
      stamp = lglstamp(lgl, root, dfpr, dfopf, &work, &units, &sccs, &trds,
  ...
  lglprt(lgl, 2, "[unhd-%d-%d] %d unassigned variables out of %d (%.0f%%)",
         lgl->stats->unhd.count, lgl->stats->unhd.rounds, lgl->unassigned,
         lgl->nvars - 2, lglpcnt(lgl->unassigned, lgl->nvars - 2));
  lglprt(lgl, 2, "[unhd-%d-%d] %d root literals out of %d (%.0f%%)",
         lgl->stats->unhd.count, lgl->stats->unhd.rounds, roots, unassigned,
         lglpcnt(roots, unassigned));
  lglprt(lgl, 2,
         "[unhd-%d-%d] %d additional non-root searches out of %d (%.0f%%)",
         lgl->stats->unhd.count, lgl->stats->unhd.rounds, searches - roots,
         unassigned, lglpcnt(searches - roots, unassigned));
  lglprt(lgl, 2, "[unhd-%d-%d] %d literals not in F2 out of %d (%.0f%%)",
         lgl->stats->unhd.count, lgl->stats->unhd.rounds, noimpls, unassigned,
         lglpcnt(noimpls, unassigned));
  lglprt(lgl, 2, "[unhd-%d-%d] %d visited literals out of %d (%.0f%%)",
         lgl->stats->unhd.count, lgl->stats->unhd.rounds, visited, unassigned,
         lglpcnt(visited, unassigned));
  lglprt(lgl, 2, "[unhd-%d-%d] %.2f average number visited literals per search",
...
static void lglsetunhdlim(LGL *lgl) {
  ...
  if (lgl->opts->irrlim.val && limit < irrlim) {
    ...
    lglprt(lgl, 1, "[unhide-%d] limit %lld with penalty %d = %d + %d",
           lgl->stats->unhd.count, (LGLL)limit, pen, lgl->limits->unhd.pen,
           szpen);
...
static int lglunhide(LGL *lgl) {
  ...
  while (!lgl->mt) {
    if (round >= maxrounds)
      break;
    if (round > 0 && roundprgss == lgl->stats->prgss &&
  ...
  lglprt(lgl, 1,
         ...
         lglunhdtauts(lgl) - oldtauts, lglunhdhbrs(lgl) - oldhbrs,
...
static void lglgaussort(LGL *lgl) {
  ...
  lglprt(lgl, 3, "[gauss-%d] sorted %d remaining variables",
...
static void lglgaussgc(LGL *lgl) {
  ...
  if (lgl->gauss->garbage < count / 2 + 10000)
    return;
  lgl->stats->gauss.gcs++;
  lglprt(lgl, 2, "[gauss-%d] collecting %d garbage out of %d",
...
static int lglgausselimvar(LGL *lgl, int pivot) {
  ...
  LOG(2, "eliminating variable %d occurring %d times", pivot,
      lglgaussoccs(lgl, pivot));
...
static void lglsetgausslim(LGL *lgl) {
  ...
  if (lgl->opts->irrlim.val && limit < irrlim) {
    ...
    lglprt(lgl, 1, "[gauss-%d] limit %lld penalty %d = %d + %d",
           lgl->stats->gauss.count, (LGLL)limit, pen, lgl->limits->gauss.pen,
           szpen);
...
static int lgladdcard(LGL *lgl, const int *lits, int bound, int *subsumed_ptr) {
  Card *card = lgl->card;
  const int *p;
  int start, lit;
#if !defined(NDEBUG) || !defined(NLGLOG)
  ...
  LOGCLS(CARDLOGLEVEL, lits, "new %s cardinality constraint %d >=",
...
static void lglcardfmstep(LGL *lgl, int pivot, int cardposidx, int cardnegidx) {
  ...
  if (lgl->opts->log.val >= CARDLOGLEVEL) {
    int first = 1;
    lglogstart(lgl, CARDLOGLEVEL, "pseudo-boolean resolvent %d - %d >= ", b,
               elim);
    for (p = lgl->clause.start; p < lgl->clause.top; p++) {
      idx = *p;
  ...
  if (!cardcut && div > 1) {
    ...
    if (b == 1 &&
        ((ln >= 3 && lp >= 3) ||
         lglcntstk(&lgl->clause) > lglfactor(lgl, lgl->stats->card.count,
...
static void lglcardfm(LGL *lgl) {
  ...
  lglcardresched(lgl);
  elim = &lgl->card->elim;
  while (!lgl->mt && !lglmtstk(elim) && !lglterminate(lgl) &&
...
static int64_t lglsetcardlim(LGL *lgl) {
  ...
  if (lgl->opts->irrlim.val && limit < irrlim) {
    ...
    lglprt(lgl, 1, "[card-%d] limit %lld penalty %d = %d + %d",
           lgl->stats->card.count, (LGLL)limit, pen, lgl->limits->card.pen,
           szpen);
...
static void lglsetcardlimagain(LGL *lgl, int64_t limit) {
  lglprt(lgl, 1, "[card-%d] using previous limit %lld for elimination",
         lgl->stats->card.count, (LGLL)limit, lgl->stats->irr.clauses.cur);
  lgl->limits->card.steps = lgl->stats->card.steps + limit;
}
...
static int lglcard1extractlit(LGL *lgl, int lit) {
  ...
  for (p = w; p < eow; p++) {
    ...
    if (i < lglcntstk(&card->atmost1))
      continue;
    card->marked[other] = 1;
    assert(i == lglcntstk(&card->atmost1));
    lglpushstk(lgl, &card->atmost1, other);
    LOG(CARDLOGLEVEL + 1, "adding %d to at-most-one clique[%d] for %d", other,
        start, lit);
  ...
  if (trivial || subsumed) {
    LOG(CARDLOGLEVEL + 1,
        "resetting at-most-one clique[%d] for %d of trivial size %d", start,
        lit, size);
...
static int lglcard1extract(LGL *lgl) {
  ...
  if (delta.cnt)
    ...
    lglprt(lgl, 1, "[card-%d] no at-most-one constraint found",
    ...
    for (j = i + 1; j < size - 1; j++) {
      ...
      for (k = j + 1; k < size; k++) {
        ...
        if (!lglhastrn(lgl, other, other2, other3)) {
          LOG(CARDLOGLEVEL + 1, "ternary clause %d %d %d missing", other,
              other2, other3);
          assert(size >= 4);
          if (size == 4)
            goto FAILED;
...
static int lglcard2extract(LGL *lgl) {
  ...
  if (delta.cnt)
    ...
    lglprt(lgl, 1, "[card-%d] no at-most-two constraint found",
           lgl->stats->card.count);
...
static int lglcardelim(LGL *lgl, int count) {
  ...
  if (used) {
    ...
    lglprt(lgl, 1, "[card-%d] no constraint out of %d used (%d subsumed)",
    ...
    for (sign = -1; sign <= 1; sign += 2) {
      ...
      for (p = w; p < eow; p++) {
        ...
        if (tag == BINCS) {
          ...
          if (abs(other2) < idx)
            continue;
          clause[0] = -lit, clause[1] = -other, clause[2] = -other2,
          clause[3] = 0;
...
SKIP:
  ...
  if (lgl->mt)
    ...
    lglprt(lgl, 1, "[card-%d] found %d units", lgl->stats->card.count,
           lglcntstk(&card->units));
    while (!lgl->mt && !lglmtstk(&card->units)) {
      lit = lglpopstk(&card->units);
      if (lglval(lgl, lit) > 0)
        continue;
      if (lglval(lgl, lit) < 0) {
        lglprt(lgl, 1, "[card-%d] found inconsistent unit",
               lgl->stats->card.count);
        lglmt(lgl);
      } else {
        lglunit(lgl, lit);
        if (!lglbcp(lgl)) {
          lglprt(lgl, 1, "[card-%d] inconsistent unit propagation",
                 lgl->stats->card.count);
          assert(!lgl->mt);
          lglmt(lgl);
        }
  ...
  if (!lgl->mt) {
    ...
    if (!lglmtstk(&card->expam1)) {
      ...
      for (c = card->expam1.start; c < card->expam1.top; c = p + 1) {
        ...
        for (p = c; (a = -*p); p++) {
          ...
          for (q = p + 1; (b = -*q); q++)
            if (!lglhasbin(lgl, a, b)) {
              LOG(CARDLOGLEVEL,
                  "exporting at-most-one constraint binary clause %d %d", a, b);
              assert(lglmtstk(&lgl->clause));
              lglpushstk(lgl, &lgl->clause, a);
              lglpushstk(lgl, &lgl->clause, b);
              lglpushstk(lgl, &lgl->clause, 0);
      ...
      lglprt(lgl, 1,
             ...
             lgl->stats->card.count, generated, genused,
      ...
      lglprt(lgl, 1, "[card-%d] no at-most-one constraints generated",
             lgl->stats->card.count);
  }
...
static void lglbca(LGL *lgl) {
  ...
  lglprt(lgl, 1, "[bca-%d] limit of %d steps and %d added (scaled %d)",
  ...
  lglprt(lgl, 2,
         ...
         lgl->stats->bca.count, inred, 2 * (lgl->nvars - 1),
  ...
  lglprt(lgl, 1, "[bca-%d] %d out of %d literals actually usable %.0f%%",
         lgl->stats->bca.count, usable, 2 * (lgl->nvars - 1),
...
DONE:
  ...
  lglprt(lgl, 1, "[bca-%d] added %d blocked clauses in %lld steps",
         lgl->stats->bca.count, added, (LGLL)lgl->stats->bca.steps - oldsteps);
  assert(lgl->bcaing);
  lgl->bcaing = 0;
...
static int lglforcerestart(LGL *lgl) {
  ...
  LOG(2, "%s glue MACD %g * %.2f = %g %s %g at %lld conflicts",
      (forcing ? "forcing" : "not forcing"), smoothed / (double)(1ll << 32),
      lgl->opts->restartforcelim.val / 100.0, limit / (double)(1ll << 32),
      forcing ? "<" : ">=", actual / (double)(1ll << 32),
      (LGLL)lgl->stats->confs);
  return forcing;
}
...
static int lgldelayrestart(LGL *lgl) {
  ...
  LOG(2, "%s restart MACD %g * %.2f = %g %s %g at %lld conflicts",
      (delaying ? "delaying" : "not delaying"), smoothed / (double)(1ll << 32),
      lgl->opts->restartdelaylim.val / 100.0, limit / (double)(1ll << 32),
      delaying ? ">" : "<=", actual / (double)(1ll << 32),
      (LGLL)lgl->stats->confs);
  return delaying;
}
...
static void lglupdlocslim(LGL *lgl, int updconflimtoo) {
  ...
  lglprt(lgl, 1 + !lgl->opts->locs.val,
         "[locs-lim] next local search variable limit %d = %d - %d", varslimit,
         rem, vardelta);
  if (!updconflimtoo)
    return;
...
static int lglocsing(LGL *lgl) {
  ...
  if (rem < lgl->limits->locs.vars) {
    lglprt(lgl, 2,
           "[locs-limit] local search skipped (remaining %d < limit %d)", rem,
           lgl->limits->locs.vars);
    lglupdlocslim(lgl, 1);
    return 0;
  }
...
static void lglocscopy(LGL *lgl, Yals *yals) {
  ...
  lglprt(lgl, 1, "[locs-%d] copied %d irredundant clauses",
         lgl->stats->locs.count, count - redcount);
  if (redcount)
    lglprt(lgl, 1, "[locs-%d] copied %d redundant clauses",
  ...
  if (count)
    lglprt(lgl, 1, "[locs-%d] copied %d assumptions as %d unit clauses",
...
static int64_t lglsetlocslim(LGL *lgl) {
  ...
  if (lgl->opts->locsrtc.val) {
    limit = LLMAX;
    lglprt(lgl, 1, "[locs-%d] really no limit (run to completion)",
    ...
    if (lgl->stats->locs.count <= 1 && lgl->opts->boost.val &&
        lgl->opts->locsboost.val > 1) {
      boost = lgl->opts->locsboost.val;
      lglprt(lgl, 1, "[locs-%d] intially boosting limit by factor of %d",
    ...
    pen = lglitszpen(lgl);
    limit >>= pen;
    lglprt(lgl, 1, "[locs-%d] limit %lld literal penalty %d",
...
static int lglocsaux(LGL *lgl, int hitlim) {
  ...
  if (!lgl->stats->locs.count++) {
    ...
    if (lgl->opts->verbose.val > 0)
      lglprt(lgl, 1, "[locs-%d] YalSAT Version %s", lgl->stats->locs.count,
             yals_version());
  ...
  yals = yals_new_with_mem_mgr(lgl, (YalsMalloc)lglnew, (YalsRealloc)lglrsz,
                               (YalsFree)lgldel);
  yals_setout(yals, lgl->out);
  len = strlen(lgl->prefix) + 80;
  NEW(prefix, len);
  if (lgl->tid >= 0)
    sprintf(prefix, "%s%d [locs-%d] ", lgl->prefix, lgl->tid,
            lgl->stats->locs.count);
  else
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->locs.count, pos, lglpcnt(pos, pos + neg), neg,
         lglpcnt(neg, pos + neg));
  if (limit < LLMAX / 1000)
    limit *= 1000;
  else
  ...
  if (lgl->cbs && lgl->cbs->msglock.lock)
    yals_setmsglock(yals, lgl->cbs->msglock.lock, lgl->cbs->msglock.unlock,
                    lgl->cbs->msglock.state);
  res = yals_sat(yals);
  min = yals_minimum(yals);
  lglprt(lgl, 1, "[locs-%d] local search returns %d with minimum %d",
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->locs.count, save ? "exported" : "found", pos,
         lglpcnt(pos, pos + neg), neg, lglpcnt(neg, pos + neg));
DONE:
...
static void lglinitsweep(LGL *lgl) {
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->sweep.count, lgl->swp->round, pos, lglpcnt(pos, rem), neg,
         lglpcnt(neg, rem), rem, lglpcnt(rem, lgl->nvars - 2), ret);
}
...
DONE:
  LOG(2, "sweep SAT environment of %d and %d, contains %d decisions", a, b,
      count);
...
static int lglsweepdec(LGL *lgl) {
  ...
  LOG(2, "sweep typed %d implication check decision %d score %s",
...
static int lglsweepsat(LGL *lgl, int lim, int a, int b) {
  ...
  lgl->stats->sweep.sat++;
  lgl->swp->sat++;
  LOG(2, "deep sweep implication SAT check %d/%d decision limit %d",
  ...
  for (;;) {
    if (lglbcp(lgl)) {
      ...
      LOG(2, "sweep typed %d assumption %d score %s", lgl->swp->type, a,
          lglscr2str(lgl, lglqvar(lgl, a)->score));
      if (!bval) {
        assert(lgl->level <= 1);
        lgliassume(lgl, -b);
        lgl->swp->decision.count++;
      ...
      LOG(2, "sweep typed %d assumption %d score %s", lgl->swp->type, b,
          lglscr2str(lgl, lglqvar(lgl, b)->score));
...
static int lglsweeplies(LGL *lgl, int a, int b) {
  ...
  lglprt(lgl, 2, "[sweep-%d-%d-%d] implication check %s",
...
static void lglsweepquery(LGL *lgl) {
  ...
  assert(count > 0);
  lglprt(lgl, 2, "[sweep-%d-%d-%d] %d classes with %d vars of avg size %.1f",
         lgl->stats->sweep.count, lgl->swp->round, lgl->swp->query,
  ...
  if (lgl->swp->partitions >= newpartitions)
    ...
    lglprt(lgl, 2, "[sweep-%d-%d-%d] removed %d literals and added %d classes",
           lgl->stats->sweep.count, lgl->swp->round, lgl->swp->query,
...
static void lglsetsweeplim(LGL *lgl) {
  ...
  if (lgl->opts->sweeprtc.val > 1) {
    ...
    lglprt(lgl, 1, "[sweep-%d] really no limit (run to completion)",
           lgl->stats->sweep.count);
  } else if (lgl->opts->sweeprtc.val || lgl->opts->prbrtc.val ||
             (count > 0 && lglrem(lgl) < lgl->opts->sweeprtcintvlim.val &&
              !(count % lgl->opts->sweeprtcint.val))) {
    limit = 4000000000ll;
    lgl->limits->sweep.steps = lgl->stats->sweep.steps + limit;
    lglprt(lgl, 1, "[sweep-%d] almost no limit of %lld steps",
    ...
    if (count > 0 && (count <= 1 || !(count % lgl->opts->sweepboostint.val)) &&
        lglrem(lgl) < lgl->opts->sweepboostvlim.val && lgl->opts->boost.val &&
        lgl->opts->sweepboost.val > 1) {
      lglprt(lgl, 1, "[sweep-%d] boosting sweeping limit by %d",
    ...
    if (lgl->opts->irrlim.val && limit < irrlim) {
      ...
      lglprt(lgl, 1,
             ...
             lgl->stats->sweep.count, (LGLL)limit, lgl->stats->irr.clauses.cur,
             szpen);
    } else
      lglprt(lgl, 1, "[sweep-%d] limit %lld penalty %d = %d + %d",
             lgl->stats->sweep.count, (LGLL)limit, pen, lgl->limits->sweep.pen,
             szpen);
    lgl->limits->sweep.steps = lgl->stats->sweep.steps + limit;
  }
}
...
static void lglsweepretain(LGL *lgl) {
  ...
  if (rem) {
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->sweep.count, lgl->swp->round, rem, lglpcnt(rem, sum),
           ret, lglpcnt(ret, sum));
  } else {
    lglprt(lgl, 1, "[sweep-%d-%d] fully completed sweeping round",
...
RESTART:
  ...
  lgl->swp->round = round;
  lglinitsweep(lgl);
  for (lgl->swp->query = 1; !lgl->mt && !lglmtstk(&lgl->swp->partition) &&
                            lgl->stats->sweep.steps <= lgl->limits->sweep.steps;
       lgl->swp->query++)
    lglsweepquery(lgl);
  deltasteps = lgl->stats->sweep.steps - oldsteps;
  lglprt(lgl, 1, "[sweep-%d-%d] %lld steps %d queries %.2f avg class size",
         lgl->stats->sweep.count, round, (LGLL)deltasteps, lgl->swp->query,
         lglavg(lgl->swp->sumsize.classes, lgl->swp->query));
  lglprt(lgl, 1, "[sweep-%d-%d] %d decisions %d SAT calls %.2f avg env size",
         lgl->stats->sweep.count, round, lgl->swp->decision.count,
         lgl->swp->sat, lglavg(lgl->swp->sumsize.envs, lgl->swp->sat));
  ...
  lglprt(lgl, 1, "[sweep-%d-%d] removed %d vars (%d failed, %d eqs, %d impls)",
         lgl->stats->sweep.count, round, deltarem, deltafailed, deltaequivs,
         deltaimpls);
  ...
  if (!lgl->sweeprtc &&
    ...
    if (!success)
      lglprt(lgl, 1, "[sweep-%d] %d < 1/%d * %d = %d considered unsuccessful",
             lgl->stats->sweep.count, deltaorigrem,
             lgl->opts->sweepsuccessrat.val, origrem,
...
static void lglqueuesort(LGL *lgl) {
  ...
  lglprt(
      ...
      (LGLL)lgl->stats->queue.sorted, count, (LGLL)lgl->stats->confs,
...
static void lglshowscoredist(LGL *lgl) {
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->simp.count, pos, lglpcnt(pos, sum), min, lglpcnt(min, sum),
  ...
  lglprt(lgl, 1,
         ...
         lgl->stats->simp.count, diff, lglpcnt(diff, sum), sum);
  lglstop(lgl);
}
...
static Features lglfeatures(LGL *lgl) {
  ...
  assert(res.b + res.t + res.q + res.c1 + res.c2 + res.c3 + res.c4 + lrg ==
         res.cc);
  return res;
}
...
static int lglsetoptif(LGL *lgl, const char *a, const char *b, const char *opt,
                       int val) {
  if (strcmp(a, b))
    return 0;
  lglprt(lgl, 1, "[classify-%d] setting '--%s=%d' since assuming '%s' bucket",
...
static void lglshowfeatures(LGL *lgl) {
  ...
  if (lgl->opts->classify.val && lgl->stats->features == 1) {
    const char *bucket =
        sc14classify(f.vo, f.vc, f.co, f.cc, f.b, f.t, f.q, f.c1, f.c2, f.c3,
                     f.c4, f.x, f.a1, f.a2, f.g, f.j, f.c, f.o);
    lglprt(lgl, 1,
           "[classify-%d] classified as SAT Competition 2014 bucket '%s'", f.n,
           bucket);
...
static void lglupdsimpcinc(LGL *lgl, int red, int success) {
  ...
  lglprt(lgl, 2, "[simplification-%d] new lgl->limits->simp.cinc %d",
  ...
  if (div > 1)
    ...
    lglprt(lgl, 1, "[simplification-%d] full %s conflict interval increment %d",
           lgl->stats->simp.count, type, res);

  if (success && lgl->opts->simpcintmaxsoft.val >= 0 &&
...
static void lglsethardsimplim(LGL *lgl) {
  ...
  lglprt(
      lgl, 1, // TODO set this to 2
      "[simplification-%d] hard conflict limit %lld (soft %lld + delta %lld)",
      lgl->stats->simp.count, (LGLL)lgl->limits->simp.hard,
      (LGLL)lgl->limits->simp.confs, (LGLL)delta);
}
...
static void lglupdsimpint(LGL *lgl, int oldrem, int oldirr, int forced) {
  ...
  lglprt(lgl, 1,
         ...
         "(%d remain %0.f%%)",
         lgl->stats->simp.count, remvar, lglpcnt(remvar, oldrem), lglrem(lgl),
  ...
  if (forced) {
    ...
    int64_t scaledcinc;
    int factor;
    lglprt(lgl, 1, "[simplification-%d] large reduction %d%% >= %d%% limit",
    ...
    if (scaledcinc < simpcinc) {
      lglprt(lgl, 1, "[simplification-%d] next conflict interval %d = %d/%d",
             lgl->stats->simp.count, simpcinc, lgl->opts->simpcintdelay.val,
             factor);
    } else {
      ...
      lglprt(
          ...
          lgl->stats->simp.count, simpcinc, (LGLL)lgl->stats->confs,
          lgl->opts->simpiscale.val, lgl->limits->simp.cinc);
    }
  } else {
  ...
  if (forced) {
    ...
    lglprt(lgl, 1, "[simplification-%d] new conflict limit %lld (hard %lld)",
           lgl->stats->simp.count, (LGLL)lgl->limits->simp.confs,
           (LGLL)lgl->limits->simp.hard);
  }
  ...
  if (forced == 1 && lgl->limits->simp.itinc < lgl->opts->simpitintinclim.val) {
    ...
    lglprt(lgl, 1, "[simplification-%d] new iteration interval %d",
  ...
  if (forced == 2 && lgl->limits->simp.binc < lgl->opts->simpbintinclim.val) {
    ...
    lglprt(lgl, 1, "[simplification-%d] new binary interval %d",
  ...
  if (forced == 3 && lgl->limits->simp.tinc < lgl->opts->simptintinclim.val) {
    ...
    lglprt(lgl, 1, "[simplification-%d] new ternary interval %d",
...
static int lglsimplimhit(LGL *lgl, int *forcedptr) {
  ...
  if (lgl->stats->confs < lgl->limits->simp.hard) {
    if (lgl->opts->simpitintdecdelay.val &&
        lgl->stats->its.avg.diff.smoothed.val < 0)
      return 0;
    if (lgl->opts->simpjleveldecdelay.val &&
        lgl->stats->jlevel.diff.smoothed.val < 0)
      return 0;
  }
  if (lgl->stats->confs >= lgl->limits->simp.confs) {
    lglprt(lgl, 1, "");
    lglprt(lgl, 1,
           ...
           lgl->stats->simp.count + 1, (LGLL)lgl->limits->simp.confs,
           (LGLL)lgl->limits->simp.hard, (LGLL)lgl->stats->confs);
  ...
  if (!res && lgl->stats->trns >= lgl->limits->simp.trn) {
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->simp.count + 1, (LGLL)lgl->limits->simp.trn,
           (LGLL)lgl->stats->trns);
  ...
  if (!res && lgl->stats->bins >= lgl->limits->simp.bin) {
    ...
    lglprt(lgl, 1,
           ...
           lgl->stats->simp.count + 1, (LGLL)lgl->limits->simp.bin,
           (LGLL)lgl->stats->bins);
  ...
  if (!res && lgl->stats->its.count >= lgl->limits->simp.its) {
    ...
    lglprt(
        ...
        lgl->stats->simp.count + 1, (LGLL)lgl->limits->simp.its,
        lgl->stats->its.count);
    lgl->stats->simp.limhit.its++;
    *forcedptr = 1;
    res = 1;
  }
  if (!res && (o = lgl->limits->simp.vars) &&
...
static int lglimhit(LGL *lgl, Lim *lim) {
  ...
  if (lim->props >= 0 && lgl->stats->props.search >= lim->props) {
    lglprt(lgl, 1, "[limits] propagation limit %lld hit at %lld propagations",
...
static int lgltopsimp(LGL *lgl, int forcesimp) {
  ...
  if ((forcesimp || lglsimplimhit(lgl, &forcesimp)) &&
      !lglpreprocessing(lgl, forcesimp))
    return 20;
  if (lglfailedass(lgl))
    return 20;
  lglrep(lgl, 2, 's');
...
static void lglincsetup(LGL *lgl) {
  ...
  lglinitmacd(lgl, &lgl->stats->jlevel, lgl->opts->jlevelmacdfast.val,
              lgl->opts->jlevelmacdslow.val, lgl->opts->jlevelmacdsmooth.val);

  lglinitema(lgl, &lgl->stats->tlevel, lgl->opts->tlevelema.val);

  lglinitmacd(lgl, &lgl->stats->glue, lgl->opts->gluemacdfast.val,
              lgl->opts->gluemacdslow.val, lgl->opts->gluemacdsmooth.val);

  lglinitavg(&lgl->stats->avglue);

  lglinitmacd(lgl, &lgl->stats->its.avg, lgl->opts->itsmacdfast.val,
              lgl->opts->itsmacdslow.val, lgl->opts->itsmacdsmooth.val);
  ...
  for (equiv = 0; equiv <= 1; equiv++) {
    ...
    for (eidx = 1; eidx <= lgl->maxext; eidx++) {
      ...
      if (ext->equiv) {
        ...
        LOG(3,
            "initializing external %d assignment from external representative "
            "%d",
            eidx, erepr);
        assert(erepr != eidx);
        ...
        if (!(val = extrepr->val)) {
          ilit = extrepr->repr;
          if (ilit) {
            LOG(3, "using external %d to internal %d mapping", abs(erepr),
                ilit);
...
static void lglclass(LGL *lgl, LGL *from) {
  ...
  lglcomputechanged(lgl);
  lglprt(lgl, 1, "[class] cloned %d assignments (%d changed)", cloned,
         lgl->changed);
  TRANS(EXTENDED);
#ifndef NCHKSOL
  lglchksol(lgl);
...
#define CHKCLONERES(FUN, RES)                                                  \
  do {                                                                         \
    ...
    ABORTIF(CLONERES != (RES),                                                 \
            "%s (lgl->clone) = %d differs from %s (lgl) = %d", __FUNCTION__,   \
            CLONERES, __FUNCTION__, (RES));                                    \
    CHKCLONE();                                                                \
  } while (0)
...
static void lglsetlim(LGL *lgl, Lim *lim) {
  ...
  if ((plim = lgl->opts->plim.val) < 0) {
    ...
    lglprt(lgl, 1, "[limits] propagation limit %lld after %lld propagations",
  ...
  if ((delay = lgl->opts->simpinitdelay.val) > 0) {
    ...
    if (delayed > lgl->limits->simp.confs) {
      ...
      lglsethardsimplim(lgl);
      lglprt(lgl, 1,
             "[limits] simplification delayed by %lld to %lld conflicts (hard "
             "%lld)",
...
void lglfreeze(LGL *lgl, int elit) {
  ...
  REQUIRE(UNUSED | OPTSET | USED | RESET | SATISFIED | UNSATISFIED | FAILED |
          LOOKED | UNKNOWN | EXTENDED);
...
static void lglgluestats(LGL *lgl) {
  ...
  int64_t avgadded, avgreduced, avgretired, avgforcing, avgresolved,
      avgconflicts;
  ...
  Stats *s = lgl->stats;
  int glue, maxglue;
  lglprs(lgl, "scaled glue%11s %3s %9s %3s %9s %3s %9s %3s %9s %3s %9s %7s",
         "added", "", "reduced", "", "retired", "", "forcing", "", "resolved",
         "", "conflicts", "maxmb");
  ...
  lglprsline(lgl);
  lglprs(lgl, "all %14lld %13lld %13lld %13lld %13lld %13lld", (LGLL)added,
         (LGLL)reduced, (LGLL)retired, (LGLL)forcing, (LGLL)resolved,
         (LGLL)conflicts);
  lglprsline(lgl);
  if (lgl->opts->verbose.val < 2) {
  ...
  for (glue = 0; glue <= maxglue; glue++) {
    lglprs(lgl,
           "%2d %-5d %9lld %3.0f%c%9lld %3.0f%c%9lld %3.0f%c%9lld %3.0f%c%9lld "
           "%3.0f%c%9lld %3.0f%c %6.1f",
           glue, lglunscaleglue(lgl, glue), (LGLL)s->lir[glue].added,
           lglpcnt(s->lir[glue].added, added), (glue == avgadded) ? '<' : ' ',
           (LGLL)s->lir[glue].reduced, lglpcnt(s->lir[glue].reduced, reduced),
           (glue == avgreduced) ? '<' : ' ', (LGLL)s->lir[glue].retired,
           lglpcnt(s->lir[glue].retired, retired),
           (glue == avgretired) ? '<' : ' ', (LGLL)s->lir[glue].forcing,
           lglpcnt(s->lir[glue].forcing, forcing),
           (glue == avgforcing) ? '<' : ' ', (LGLL)s->lir[glue].resolved,
           lglpcnt(s->lir[glue].resolved, resolved),
           (glue == avgresolved) ? '<' : ' ', (LGLL)s->lir[glue].conflicts,
  ...
  madded = lglavg(wadded, added), mreduced = lglavg(wreduced, reduced),
  mretired = lglavg(wretired, retired), mforcing = lglavg(wforcing, forcing),
  mresolved = lglavg(wresolved, resolved),
  mconflicts = lglavg(wconflicts, conflicts);

  lglprs(lgl, "avg  %19.1f%14.1f%14.1f%14.1f%14.1f%14.1f", madded, mreduced,
         mretired, mforcing, mresolved, mconflicts);
  ...
  lglprs(lgl, "std  %19.1f%14.1f%14.1f%14.1f%14.1f%14.1f", sadded, sreduced,
         sretired, sforcing, sresolved, sconflicts);
}
...
static void lglprofsort(LGL *lgl) {
  ...
  for (i = 0; i < nprint; i++) {
    ...
    lglprs(lgl, "%8.3f %3.0f%% %s", tns[i].t, lglpcnt(tns[i].t, ts->all),
           tns[i].n);
  }
...
static void lglprof(LGL *lgl) {
  ...
  if (!lglignprofptr(lgl, &ts->prb.simple) &&
      (lgl->opts->profilelong.val || lgl->opts->verbose.val >= 2 ||
       ts->prb.simple || ts->prb.basic || ts->prb.treelook)) {
    lglprs(lgl, "----------------------------------");
    lglprs(lgl, "%8.3f %3.0f%% probe simple    %3.0f%%", ts->prb.simple,
           lglpcnt(ts->prb.simple, t), lglpcnt(ts->prb.simple, ts->probe));
    lglprs(lgl, "%8.3f %3.0f%% probe basic     %3.0f%%", ts->prb.basic,
           lglpcnt(ts->prb.basic, t), lglpcnt(ts->prb.basic, ts->probe));
    lglprs(lgl, "%8.3f %3.0f%% probe tree-look %3.0f%%", ts->prb.treelook,
           lglpcnt(ts->prb.treelook, t), lglpcnt(ts->prb.treelook, ts->probe));
  }
  lglprs(lgl, "==================================");
  lglprs(lgl, "%8.3f %3.0f%% preprocessing   %3.0f%%", ts->preprocessing,
         lglpcnt(ts->preprocessing, t), lglpcnt(ts->preprocessing, simp));
  lglprs(lgl, "%8.3f %3.0f%% inprocessing    %3.0f%%", ts->inprocessing,
         lglpcnt(ts->inprocessing, t), lglpcnt(ts->inprocessing, simp));
  lglprs(lgl, "==================================");
SHORT:
  lglprs(lgl, "%8.3f %3.0f%% simplifying", simp, lglpcnt(simp, t));
  if (lgl->stats->calls.lkhd)
    lglprs(lgl, "%8.3f %3.0f%% lookahead", ts->lookahead,
...
void lglstats(LGL *lgl) {
  ...
  lglprs(lgl, "blkd: %d blocking literals %.0f%%, %d pure", s->blk.lits,
         lglpcnt(s->blk.lits, 2 * lgl->maxext), s->blk.pure);

  lglprs(lgl, "bump: %lld bumped seen, %lld lits %.1f on average",
         s->bump.count, (LGLL)s->bump.lits,
         lglavg(s->bump.lits, s->bump.count));

  lglprs(lgl, "card: %d count, %d units, %d am1, %lld steps", s->card.count,
         s->card.units, s->card.expam1, (LGLL)s->card.steps);
  lglprs(lgl,
  ...
  lglprs(lgl, "cces: %d cces, %d eliminated, %d ate %.0f%%, %d abce %.0f%%",
         s->cce.count, s->cce.eliminated, s->cce.ate,
         lglpcnt(s->cce.ate, s->cce.eliminated), s->cce.abce,
         lglpcnt(s->cce.abce, s->cce.eliminated));
  lglprs(lgl, "cces: %lld probed, %d lifted, %d failed", (LGLL)s->cce.probed,
         s->cce.lifted, s->cce.failed);
  lglprs(lgl,
         "cces: %lld cached, %lld rsz, %lld cols %.0f%%, "
         "%lld srch, %lld hits %.0f%%",
         (LGLL)s->cce.cache.ins, (LGLL)s->cce.cache.rsz,
         (LGLL)s->cce.cache.cols, lglpcnt(s->cce.cache.cols, s->cce.cache.ins),
         (LGLL)s->cce.cache.search, (LGLL)s->cce.cache.hits,
         lglpcnt(s->cce.cache.hits, s->cce.cache.search));

  lglprs(lgl, "clls: %lld sat, %lld simp, %lld freeze, %lld melt",
         (LGLL)s->calls.sat, (LGLL)s->calls.simp, (LGLL)s->calls.freeze,
         (LGLL)s->calls.melt);
  lglprs(lgl, "clls: %lld add, %lld assume, %lld deref, %lld failed",
         (LGLL)s->calls.add, (LGLL)s->calls.assume, (LGLL)s->calls.deref,
         (LGLL)s->calls.failed);
  lglprs(lgl, "clls: %lld lkhd, %lld cassume, %lld mosat", (LGLL)s->calls.lkhd,
         (LGLL)s->calls.cassume, (LGLL)s->calls.mosat);

  lglprs(lgl, "coll: %d gcs, %d rescored clauses, %d rescored vars", s->gcs,
         s->rescored.clauses, s->rescored.vars);

  lglprs(lgl, "dcps: %d decompositions, %d equivalent %.0f%%", s->decomps,
         s->equiv.sum, lglpcnt(s->equiv.sum, lgl->maxext));

  lglprs(lgl, "deco: %lld decision-only clauses %.0f%%, %.0f%% reduction",
  ...
  lglprs(
      ...
      (LGLL)s->decisions, (LGLL)s->randecs, lglpcnt(s->randecs, s->decisions),
      (LGLL)s->hdecs, lglpcnt(s->hdecs, s->decisions), (LGLL)s->qdecs,
      lglpcnt(s->qdecs, s->decisions));

  lglprs(lgl, "elms: %d elims, %d eliminated %.0f%%, %d pure %.0f%%",
         s->elm.count, s->elm.elmd, lglpcnt(s->elm.elmd, lgl->maxext),
         s->elm.pure, lglpcnt(s->elm.pure, s->elm.elmd));
  lglprs(lgl, "elms: %d rounds, %.1f rounds/bve", s->elm.rounds,
         lglavg(s->elm.rounds, s->elm.count));
  lglprs(lgl, "elms: %d small %.0f%%, %d large %.0f%%", s->elm.small.elm,
         lglpcnt(s->elm.small.elm, s->elm.elmd), s->elm.large,
  ...
  lglprs(lgl, "elms: %d tried small, %d succeeded %.0f%%, %d failed %.0f%%",
         ...
         s->elm.small.failed, lglpcnt(s->elm.small.failed, s->elm.small.tried));
  lglprs(lgl, "elms: %d subsumed, %d strengthened, %d blocked", s->elm.sub,
         s->elm.str, s->elm.blkd);
  lglprs(lgl, "elms: %lld copies, %lld resolutions, %lld ipos",
         (LGLL)s->elm.copies, (LGLL)s->elm.resolutions, (LGLL)s->elm.ipos);
  lglprs(lgl, "elms: %lld subchks, %lld strchks", (LGLL)s->elm.subchks,
         (LGLL)s->elm.strchks);

  lglprs(lgl, "gaus: %d extractions, %lld extracted, %.1f size, %d max",
         s->gauss.count, s->gauss.extracted.total,
         lglavg(s->gauss.arity.sum.total, s->gauss.extracted.total),
         s->gauss.arity.max);
  lglprs(lgl, "gaus: %lld exactly-1 constraints of average arity %.1f",
  ...
  lglprs(lgl, "gaus: %d gc, %lld steps, %lld extr %.0f%%, %lld elim %.0f%%",
         s->gauss.gcs, (LGLL)steps, s->gauss.steps.extr,
         lglpcnt(s->gauss.steps.extr, steps), s->gauss.steps.elim,
         lglpcnt(s->gauss.steps.elim, steps));

  lglprs(lgl, "glue: %.1f orig avg, %.1f real avg, %.1f scaled avg",
  ...
  lglprs(lgl, "glue: %lld maxscaledglue=%d (%.0f%%), %lld kept %.0f%%",
  ...
  lglprs(lgl,
         "hbrs: %d = %d simple %.0f%% + %d trn %.0f%% + %d lrg %.0f%%, %d sub "
         "%.0f%%",
         s->hbr.cnt, s->hbr.simple, lglpcnt(s->hbr.simple, s->hbr.cnt),
         s->hbr.trn, lglpcnt(s->hbr.trn, s->hbr.cnt), s->hbr.lrg,
         lglpcnt(s->hbr.lrg, s->hbr.cnt), s->hbr.sub,
         lglpcnt(s->hbr.sub, s->hbr.cnt));

  lglprs(lgl, "locs: %d count, %lld flips, %lld mems, %d minimun",
  ...
  lglprs(lgl, "lrnd: %lld clauses, %.1f length", (LGLL)s->clauses.learned,
         lglavg(s->lits.learned, s->clauses.learned));
  lglprs(lgl, "lrnd: %lld lits = %lld orig - %lld removed %.0f%%",
         (LGLL)s->lits.learned, (LGLL)s->lits.nonmin, (LGLL)min,
         lglpcnt(min, s->lits.nonmin));
  lglprs(lgl, "lrnd: %lld uips %.0f%%, %lld flips %.0f%%", (LGLL)s->uips,
         lglpcnt(s->uips, s->clauses.learned), (LGLL)s->decflipped,
         lglpcnt(s->decflipped, s->clauses.learned));

  sum = s->mincls.min + s->mincls.bin + s->mincls.size + s->mincls.deco;
  lglprs(lgl, "mins: %lld mins %.0f%%, %lld bin %.0f%%, %lld size %.0f%%",
         (LGLL)s->mincls.min, lglpcnt(s->mincls.min, sum), (LGLL)s->mincls.bin,
         lglpcnt(s->mincls.bin, sum), (LGLL)s->mincls.size,
         lglpcnt(s->mincls.size, sum), (LGLL)s->mincls.deco,
         lglpcnt(s->mincls.deco, sum));
  lglprs(lgl, "mins: %lld poisoned searches, %lld hits, %.0f%% hit rate",
         (LGLL)s->mincls.poison.search, (LGLL)s->mincls.poison.hits,
         lglpcnt(s->mincls.poison.hits, s->mincls.poison.search));
  lglprs(lgl, "mins: %lld used twice searches, %lld hits, %.0f%% hit rate",
  ...
  lglprs(lgl, "move: moved %lld, %lld bin %.0f%%, %lld trn %.0f%%", sum,
         (LGLL)s->moved.bin, lglpcnt(s->moved.bin, sum), (LGLL)s->moved.trn,
  ...
  lglprs(lgl,
         ...
         sum, (LGLL)s->otfs.str.bin, lglpcnt(s->otfs.str.bin, sum),
  ...
  lglprs(lgl, "otfs: sub %lld, %lld red %.0f%%, %lld irr %.0f%%", sum,
  ...
  lglprs(lgl,
         ...
         sum, (LGLL)s->otfs.sub.bin, lglpcnt(s->otfs.sub.bin, sum),
  ...
  lglprs(lgl,
         ...
         (LGLL)s->phase.count, (LGLL)s->phase.set, (LGLL)s->phase.pos,
         lglpcnt(s->phase.pos, s->phase.set), (LGLL)s->phase.neg,
         lglpcnt(s->phase.neg, s->phase.set));
  lglprs(lgl, "phas: %lld flipped decision phases randomly %.0f%%",
         (LGLL)s->randphases, lglpcnt(s->randphases, s->decisions));

  lglprs(lgl, "prbs: %d basic, %lld probed, %d failed, %d lifted",
         s->prb.basic.count, (LGLL)s->prb.basic.probed, s->prb.basic.failed,
         s->prb.basic.lifted);
  lglprs(lgl, "prbs: %d simple, %lld probed, %d failed, %d eqs",
         s->prb.simple.count, (LGLL)s->prb.simple.probed, s->prb.simple.failed,
         s->prb.simple.eqs);
  lglprs(lgl, "prbs: %d treelook, %lld probed, %d failed, %d lifted",
         s->prb.treelook.count, (LGLL)s->prb.treelook.probed,
         s->prb.treelook.failed, s->prb.treelook.lifted);

  lglprs(lgl, "prom: %lld locked %.0f%% out of %lld tried",
         (LGLL)s->promote.locked, lglpcnt(s->promote.locked, s->promote.tried),
         (LGLL)s->promote.tried);

  lglprs(lgl, "prps: %lld props, %.0f props/dec", (LGLL)p,
         lglavg(s->props.search, s->decisions));
  lglprs(lgl, "prps: %.0f%% srch, %.0f%% simp, %.0f%% lkhd",
         lglpcnt(s->props.search, p), lglpcnt(s->props.simp, p),
         lglpcnt(s->props.lkhd, p));

  lglprs(lgl, "quat: %d count, %lld quat, %lld self1, %lld self2, %lld dup",
         s->quatres.count, (LGLL)s->quatres.quat, (LGLL)s->quatres.self1,
         (LGLL)s->quatres.self2, (LGLL)s->quatres.dup);

  lglprs(lgl, "queu: %lld flushed, %lld sorted, %lld decisions %.3f%%",
         (LGLL)s->queue.flushed, (LGLL)s->queue.sorted, (LGLL)s->qdecs,
         lglpcnt(s->qdecs, s->decisions));

  lglprs(
      lgl, "redc: %lld clauses %.0f%%, %lld tried %.0f%%, %lld sat %.0f%%",
      (LGLL)s->redcls.cls.red, lglpcnt(s->redcls.cls.red, s->redcls.cls.tried),
      (LGLL)s->redcls.cls.tried, lglpcnt(s->redcls.cls.tried, s->confs),
      (LGLL)s->redcls.cls.sat, lglpcnt(s->redcls.cls.sat, s->redcls.cls.tried));
  lglprs(lgl, "redc: %lld lits %.0f%%, %lld tried %.0f%%, %lld sat %.0f%%",
  ...
  lglprs(lgl, "redc: %lld times jump level reduced, %.1f levels on average",
         (LGLL)s->redcls.jlevel.red,
         lglavg(s->redcls.jlevel.sum, s->redcls.jlevel.red));

  lglprs(lgl, "reds: %d count, %.1f conflicts per reduce, %d reset",
         s->reduced.count, lglavg(s->confs, s->reduced.count),
         s->reduced.reset);
  lglprs(lgl, "reds: %d memlim %.0f%%, %d arith %.0f%%", s->reduced.memlim,
         lglpcnt(s->reduced.memlim, s->reduced.count), s->reduced.arith,
         lglpcnt(s->reduced.arith, s->reduced.count));
  sum = s->reduced.collected + s->reduced.retired;
  lglprs(lgl, "reds: %lld collected %.0f%%, %lld retired %.0f%%",
         s->reduced.collected, lglpcnt(s->reduced.collected, sum),
         s->reduced.retired, lglpcnt(s->reduced.retired, sum));

  lglprs(lgl, "rmbd: %d removed, %d red %.0f%%", s->bindup.removed,
         s->bindup.red, lglpcnt(s->bindup.red, s->bindup.removed));

  lglprs(lgl, "rsts: %lld restarts %.0f%% of checked, %lld skipped %.0f%%",
  ...
  lglprs(lgl, "rsts: %lld checks %.0f%%, %lld forced %.0f%%",
         (LGLL)s->restarts.checked, lglpcnt(s->restarts.checked, s->confs),
         (LGLL)s->restarts.forced,
         lglpcnt(s->restarts.forced, s->restarts.checked));
  lglprs(lgl, "rsts: %lld not forced (%.0f%% of checked, %.0f%% of skipped)",
         (LGLL)s->restarts.notforced,
         lglpcnt(s->restarts.notforced, s->restarts.checked),
         lglpcnt(s->restarts.notforced, s->restarts.skipped));
  lglprs(lgl, "rsts: %lld agile %.0f%% of skipped, %lld delayed %.0f%%",
  ...
  lglprs(lgl, "rsts: %lld blocked %.2f%% restart, %lld reused %.0f%%",
  ...
  lglprs(lgl, "scic: %lld set, %.1f%% min, %.1f%% max",
         (LGLL)s->setscincf.count, s->setscincf.min / 10.0,
         s->setscincf.max / 10.0);

  lglprs(lgl, "simp: %d count, %d confs %0.f%%, %d vars %.0f%%", s->simp.count,
         s->simp.limhit.confs, lglpcnt(s->simp.limhit.confs, s->simp.count),
         s->simp.limhit.vars, lglpcnt(s->simp.limhit.vars, s->simp.count));
  lglprs(lgl, "simp: %d its %0.f%%, %d bin %.0f%%, %d trns %.0f%%",
  ...
  lglprs(lgl, "swps: %d count, %lld rounds, %lld queries (%.1f/round)",
         s->sweep.count, (LGLL)s->sweep.rounds, (LGLL)s->sweep.queries.total,
         lglavg(s->sweep.queries.total, s->sweep.rounds));
  lglprs(lgl, "swps: %lld sat calls, %lld unsat (%.1f%% success rate)",
         s->sweep.sat, s->sweep.unsat, lglavg(s->sweep.confs, s->sweep.sat));
  lglprs(lgl, "swps: %lld decisions %.1f, %lld conflicts %.1f",
         (LGLL)s->sweep.decs, lglavg(s->sweep.decs, s->sweep.sat),
         (LGLL)s->sweep.confs, lglavg(s->sweep.confs, s->sweep.sat));
  lglprs(lgl, "swps; %.1f avg class size, %.1f avg environment size",
         lglavg(s->sweep.sumsize.classes, s->sweep.queries.total),
         lglavg(s->sweep.sumsize.envs, s->sweep.sat));
  lglprs(lgl, "swps: %d failed, %d impls, %d equivs", s->sweep.failed,
         s->sweep.impls, s->sweep.equivs);

  lglprs(
      lgl, "swp0: %lld type 0 queries, %lld unsat (%.1f%%), %lld sat (%.1f%%)",
  ...
  lglprs(
      lgl, "swp1: %lld type 1 queries, %lld unsat (%.1f%%), %lld sat (%.1f%%)",
  ...
  lglprs(
      lgl, "swp2: %lld type 2 queries, %lld unsat (%.1f%%), %lld sat (%.1f%%)",
  ...
  lglprs(lgl, "sync: %lld produced units, %lld successful consumptions %.0f%%",
  ...
  lglprs(
      ...
      s->sync.units.consumed.calls, s->sync.units.consumed.tried,
  ...
  lglprs(
      ...
      s->sync.cls.consumed.calls, s->sync.cls.consumed.tried,
  ...
  lglprs(lgl, "tops: %d fixed %.0f%%, %d its, %.2f confs/it", s->fixed.sum,
         lglpcnt(s->fixed.sum, lgl->maxext), s->its.count,
         lglavg(s->confs, s->its.count));

  lglprs(lgl, "trds: %d transitive reductions, %d removed, %d failed",
         s->trd.count, s->trd.red, s->trd.failed);
  lglprs(lgl, "trds: %lld nodes, %lld edges, %lld steps", (LGLL)s->trd.lits,
         (LGLL)s->trd.bins, (LGLL)s->trd.steps);

  lglprs(lgl, "trim: %lld words, %.1f MB", (LGLL)s->trims,
         (s->trims * sizeof(int)) / (double)(1 << 20));

  lglprs(lgl, "trnr: %d count, %d bin, %d trn, %lld steps", s->ternres.count,
         s->ternres.bin, s->ternres.trn, (LGLL)s->ternres.steps);

  lglprs(lgl, "unhd: %d count, %d rounds, %lld steps", s->unhd.count,
         s->unhd.rounds, (LGLL)s->unhd.steps);
  ...
  lglprs(lgl, "unhd: %lld units, %d bin, %d trn, %d lrg", sum,
         s->unhd.units.bin, s->unhd.units.trn, s->unhd.units.lrg);
  sum = lglunhdfailed(lgl);
  lglprs(lgl, "unhd: %lld failed, %d stamp, %d lits, %d bin, %d trn, %d lrg",
         sum, s->unhd.stamp.failed, s->unhd.failed.lits, s->unhd.failed.bin,
         s->unhd.failed.trn, s->unhd.units.lrg);
  sum = lglunhdtauts(lgl);
  lglprs(lgl, "unhd: %lld tauts, %d bin %.0f%%, %d trn %.0f%%, %d lrg %.0f%%",
         sum, s->unhd.tauts.bin, lglpcnt(s->unhd.tauts.bin, sum),
         s->unhd.tauts.trn, lglpcnt(s->unhd.tauts.trn, sum), s->unhd.tauts.lrg,
         lglpcnt(s->unhd.tauts.lrg, sum));
  lglprs(lgl, "unhd: %lld tauts, %d stamp %.0f%%, %d red %.0f%%", sum,
         s->unhd.stamp.trds, lglpcnt(s->unhd.stamp.trds, sum),
         s->unhd.tauts.red, lglpcnt(s->unhd.tauts.red, sum));
  sum = lglunhdhbrs(lgl);
  lglprs(lgl, "unhd: %lld hbrs, %d trn %.0f%%, %d lrg %.0f%%, %d red %.0f%%",
         sum, s->unhd.hbrs.trn, lglpcnt(s->unhd.hbrs.trn, sum),
         s->unhd.hbrs.lrg, lglpcnt(s->unhd.hbrs.lrg, sum), s->unhd.hbrs.red,
         lglpcnt(s->unhd.hbrs.red, sum));
  sum = lglunhdstrd(lgl);
  lglprs(lgl,
         "unhd: %lld str, %d bin %.0f%%, %d trn %.0f%%, %d lrg %.0f%%, %d red "
         "%.0f%%",
         sum, s->unhd.units.bin, lglpcnt(s->unhd.units.bin, sum),
         s->unhd.str.trn, lglpcnt(s->unhd.str.trn, sum), s->unhd.str.lrg,
         lglpcnt(s->unhd.str.lrg, sum), s->unhd.str.red,
  ...
  lglprs(lgl, "vars: %d remaining %.0f%% and %d removed %.0f%% out of %d",
         remaining, lglpcnt(remaining, lgl->maxext), removed,
         lglpcnt(removed, lgl->maxext), lgl->maxext);
  lglprs(lgl,
         "vars: %d fixed %.0f%%, %d eliminated %.0f%%, %d equivalent %.0f%%",
         s->fixed.sum, lglpcnt(s->fixed.sum, lgl->maxext), s->elm.elmd,
         lglpcnt(s->elm.elmd, lgl->maxext), s->equiv.sum,
  ...
  lglprs(lgl, "wchs: %lld pushed, %lld enlarged, %d defrags", (LGLL)s->pshwchs,
         (LGLL)s->enlwchs, s->defrags);
...
SHORT:
  lglprof(lgl);
  lglprsline(lgl);
  lglprs(lgl, "%13lld conflicts,    %10.1f confs/sec", (LGLL)s->confs,
         lglavg(s->confs, t));
  lglprs(lgl, "%13lld ternaries,    %10.1f confs/ternary", (LGLL)s->trns,
         lglavg(s->confs, s->trns));
  lglprs(lgl, "%13lld binaries,     %10.1f confs/binary", (LGLL)s->bins,
         lglavg(s->confs, s->bins));
  lglprs(lgl, "%13lld iterations,   %10.1f confs/iteration", (LGLL)s->its.count,
         lglavg(s->confs, s->its.count));
  lglprsline(lgl);
  lglprs(lgl, "%13lld reductions,   %10.1f redus/sec, %8.1f confs/reduction",
         (LGLL)s->reduced.count, lglavg(s->reduced.count, t),
         lglavg(s->confs, s->reduced.count));
  lglprs(lgl, "%13lld restarts,     %10.1f rests/sec, %8.1f confs/restart",
         (LGLL)s->restarts.count, lglavg(s->restarts.count, t),
         lglavg(s->confs, s->restarts.count));
  lglprs(lgl, "%13lld decisions,    %10.1f decis/sec, %8.1f decis/conflict",
         (LGLL)s->decisions, lglavg(s->decisions, t),
         lglavg(s->decisions, s->confs));
  lglprs(lgl, "%13lld propagations, %10.1f props/sec, %8.1f props/decision",
...
static void lglictrav(LGL *lgl, int internal, void *state,
                      void (*trav)(void *, int)) {
  int idx, sign, lit, blit, tag, red, other, other2;
  const int *p, *w, *eow, *c;
  HTS *hts;
  ...
  for (idx = 2; idx < lgl->nvars; idx++)
    for (sign = -1; sign <= 1; sign += 2) {
      ...
      for (p = w; p < eow; p++) {
        ...
        if (other2)
          trav(state, internal ? other2 : lglexport(lgl, other2));
        trav(state, 0);
      }
    }
  for (c = lgl->irr.start; c < lgl->irr.top; c = p + 1) {
...
LGL *lglfork(LGL *parent) {
  ...
  (void)lglbcp(parent);
  lglgc(parent);
  child = lglminit(parent->mem->state, parent->mem->alloc, parent->mem->realloc,
                   parent->mem->dealloc);
  child->parent = parent;
