...
LGL *setup_lgl(PyObject *args, PyObject *kwds) {
  ...
  static char *kwlist[] = {"clauses",      "vars",   "verbose",   "seed",
                           "simplify",     "randec", "randecint", "randphase",
                           "randphaseint", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|iiiiiiii:(iter)solve", kwlist,
                                   &clauses, &vars, &verbose, &seed, &simplify,
                                   &randec, &randecint, &randphase,
                                   &randphaseint)) {
    return NULL;
  }
...
PyDoc_STRVAR(solve_doc, "solve(clauses [, kwargs]) -> list\n\
\n\
...
static PyTypeObject SolIter_Type = {
    PyObject_HEAD_INIT(NULL) // ob_head
    0,                       // ob_size
    "soliterator",           // tp_name
